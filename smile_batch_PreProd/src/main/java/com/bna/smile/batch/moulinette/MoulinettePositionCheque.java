...
public class MoulinettePositionCheque extends AbstractJob {
	...
	public MoulinettePositionCheque(Batch mainFrame) {
				...
				for (Iterator it1 = listAgences.iterator(); it1.hasNext();) {
				 ...
				 //	if (journeeStructureBatchRetour != null&& journeeStructureBatchRetour.getCodStatJsb().intValue() == 0)
					//{
						...
						mainFrame.getEnd_exec_label().setText("Position Agence   : "+compensationVo.getStrutcure().getCodStrcStrc());
...
//				}
				...
				logger.debug("FIN POSITION COMPENSATION");
				...
				mainFrame.getEnd_exec_label().setText("Position Chéque exécuté avec succès");
			...
			StringBuffer text = new StringBuffer("Erreur dans MoulinettePositionCompensation : ");
	...
	private void print(CompensationVo compensationVo) throws IOException {
		// print etat
			...
			edit.setListe(compensationDAO.calculGlobalAgence(DateHandler.dateToStr(compensationVo.getDateComptable()),"30"));
			List<SwingInfoVo> liste=edit.getListe();
				...
				try {
					...
					OutputStream output = new FileOutputStream(new File(currentDirectory.getCanonicalPath()+ "/jasper/JasperReport.pdf")); 
...
//    private GestionRejetService gestionRejetService = (GestionRejetService) context.getBean("gestionRejetService");
//    private CompensationVo compensationVo = new CompensationVo();
//    PosExec(final CompensationVo vo) {
//		   this.compensationVo =vo;
//   }
//   public void run() {
//		   logger.info("Debut position Agence : " +compensationVo.getStrutcure().getCodStrcStrc());
//	       gestionRejetService.positionCompensation(this.compensationVo);
//		   logger.info("Fin position Agence : " +compensationVo.getStrutcure().getCodStrcStrc());
//
//
//	
//
