...
import com.bna.smile.batch.moulinette.MoulinetteAFBControlM;
...
public class MoulinetteAFBTControlMTest implements Runnable {

	private static final Logger logger = Logger.getLogger(MoulinetteAFBTControlMTest.class);
	...
	public MoulinetteAFBTControlMTest() {
		super();
	}
	...
	public void run() {
		try {
			...
			System.out.println("TextDateDebut : " + dateFormat.format(new Date()));
			System.out.println("TextDateFin : " + dateFormat.format(new Date()));
			System.out.println("Societe n° : 99");
			String strDateFichier = "";

			Date dateDebut = new Date();
			Date dateFin = new Date();
			;
			Long codeSociete = 99l; // 3S
			Date dateFichier = null;
			try {
				dateDebut = formaterDate.parse(dateFormat.format(new Date()));
				dateFin = formaterDate.parse(dateFormat.format(new Date()));
			...
			if (listeSocietesAFB != null && listeSocietesAFB.size() > 0) {
				...
				for (SocietesAFBView societesAFBView : listeSocietesAFB) {
					...
					if (codeSociete.equals(99L)) {
						logger.info("societesAFBView " + societesAFBView.getNomSoctAFB() + "==> dateComptableAgence "
								+ formaterDate.format(dateFichier));
						...
						logger.info("societesAFBView " + societesAFBView.getNomSoctAFB() + "==> dateComptableAgence "
								+ formaterDate.format(dateComptableFichier));
					...
					// ********Batch *************//
					MoulinetteAFBControlM moulinette = new MoulinetteAFBControlM(AFBVo);
			...
			System.out.println("Moulinette génération fichier AFB terminée avec succès");
