...
import com.bna.smile.batch.moulinette.MoulinetteMAJNSIActel;
...
public class MoulinetteMAJNSIActelTest implements Runnable {

	private static final Logger logger = Logger.getLogger(MoulinetteVirementsAecheanceTest.class);
	private BatchMAJNSIActelFrame frame = null;
	private static final SimpleDateFormat formaterDate = new SimpleDateFormat("dd/MM/yyyy");

	public BatchMAJNSIActelFrame getFrame() {
		return frame;
	}

	public void setFrame(BatchMAJNSIActelFrame frame) {
		this.frame = frame;
	}

	public MoulinetteMAJNSIActelTest() {
		super();
	}

	public MoulinetteMAJNSIActelTest(BatchMAJNSIActelFrame frame) {
		super();
		this.frame = frame;
	}
	...
	public void run() {
		...
		if (batchVo.getListeTraceBatch() != null && batchVo.getListeTraceBatch().size() > 0) {
			frame.getEnd_exec_label().setText("Une autre moulinette MAJ NSI  Actel est en cours d'exécution");
			...
			if (traceBatch.getNumSeqTrc() != null) {
				...
				try {
					MoulinetteMAJNSIActel moulinette = new MoulinetteMAJNSIActel(frame);
					moulinette.perform();
					...
					frame.getEnd_exec_label().setText("Moulinette MAJ NSI Actel terminée avec succès");
					frame.getBtnExcuter().setEnabled(true);
