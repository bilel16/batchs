...
public class MoulinetteInsertingChequeAgenceACH {
	...
	public MoulinetteInsertingChequeAgenceACH(Batch mainFrame) {
		this.mainFrame = mainFrame;
	}

	public IValueObject perform(IValueObject arg0) {
		List listAgences = compensationDAO.getListAgencesCompensationPiloteAgence();
			...
			for (Iterator it1 = listAgences.iterator(); it1.hasNext();) {
				...
				if ((ListAg.getValue(0)).toString() != null && (ListAg.getValue(1)).toString() != null) {
					...
					if (compensationVo.getDescription() == null || compensationVo.getDescription().equals("")) {
						...
						SuivFileTrt.validInsertAgence(compensationVo.getDateComptable(), bctAge);
					} else {
