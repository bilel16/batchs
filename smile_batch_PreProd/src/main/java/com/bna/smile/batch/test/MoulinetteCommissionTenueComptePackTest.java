...
import com.bna.smile.batch.moulinette.MoulinetteCommissionFraisTenueCompte;
...
public class MoulinetteCommissionTenueComptePackTest implements Runnable {

	private static final Logger logger = Logger.getLogger(MoulinetteCommissionTenueComptePackTest.class);
	private BatchCommissionSouscriptionPackFrame frame = null;

	private static final SimpleDateFormat formaterDate = new SimpleDateFormat("dd/MM/yyyy");

	// *********************************//

	public MoulinetteCommissionTenueComptePackTest() {
		super();
	}

	public MoulinetteCommissionTenueComptePackTest(BatchCommissionSouscriptionPackFrame frame) {
		super();
		this.frame = frame;
	}
	...
	public void run() {
		...
		// *****Verification du l'existance d'un batch en cours ********//
		...
		if (batchVo.getListeTraceBatch() != null && batchVo.getListeTraceBatch().size() > 0) {
			frame.getEnd_exec_label()
					.setText("Une autre moulinette perception frais tenue compte pack est en cours d'exécution");
			...
			batchMetierVirement.setCodBatBmet(Constants.COD_BATCH_FRAIS_TENUE_CPTE_PACK);
			...
			if (traceBatch.getNumSeqTrc() != null) {
				...
				if (listAgences != null && listAgences.size() > 0) {
					logger.info("*** Debut MoulinettePrelevementsFraisTenueCompte ***");
					...
					for (Object result : listAgences) {
						...
						// ********Batch *************//
						MoulinetteCommissionFraisTenueCompte moulinette = new MoulinetteCommissionFraisTenueCompte(virementVo);
				...
				frame.getEnd_exec_label().setText("Moulinette Commission Frais Pack terminée avec succès");
				frame.getBtnExcuter().setEnabled(true);
				logger.info("*** FIN MoulinetteCommissionTenueComptePack ***");
			}
		}
	}

	public BatchCommissionSouscriptionPackFrame getFrame() {
		return frame;
	}

	public void setFrame(BatchCommissionSouscriptionPackFrame frame) {
		this.frame = frame;
	}
}
