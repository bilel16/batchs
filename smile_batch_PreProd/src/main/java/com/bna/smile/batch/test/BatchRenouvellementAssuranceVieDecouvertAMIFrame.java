...
public class BatchRenouvellementAssuranceVieDecouvertAMIFrame extends JFrame implements Serializable {
	...
	public BatchRenouvellementAssuranceVieDecouvertAMIFrame frame_Instance = null;
	private JRadioButton radio_Assurance;
		...
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					...
					BatchRenouvellementAssuranceVieDecouvertAMIFrame frame = new BatchRenouvellementAssuranceVieDecouvertAMIFrame();
	 ...
	 * Create the frame.
	 */
	public BatchRenouvellementAssuranceVieDecouvertAMIFrame() {

		setTitle("Batch Résiliation/Renouvellement Assurance Vie Sur Découvert (AMI)");
		...
		btnExcuter.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (radio_Assurance.isSelected()) {
					btnExcuter.setEnabled(false);
					end_exec_label.setText("");
					new Thread(new MoulinetteRenouvellementAdhesionVieDecouvertAMITest(frame_Instance)).start();
		...
		radio_Assurance = new JRadioButton("Résiliation/Renouvellement Assurance Découvert");
		radio_Assurance.setSelected(true);
		buttonGroup.add(radio_Assurance);
		radio_Assurance.setBounds(35, 94, 309, 23);
		panel.add(radio_Assurance);
