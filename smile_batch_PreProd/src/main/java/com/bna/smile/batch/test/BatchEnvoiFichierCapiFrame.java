...
public class BatchEnvoiFichierCapiFrame extends JFrame implements Serializable {
	...
	public BatchEnvoiFichierCapiFrame frame_Instance = null;
		...
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					...
					BatchEnvoiFichierCapiFrame frame = new BatchEnvoiFichierCapiFrame();
	...
	public void addOrUpdateEtat(SwingInfoVo vo) {
		...
		for (int i = 0; i < infoVos.size(); i++) {
			...
			etatList[i][0] = infoVos.get(i).getEtat();
			etatList[i][1] = infoVos.get(i).getDateComptable();
			etatList[i][2] = infoVos.get(i).getInfo();
		}
		String[] columnNames = {  "Etat d'exécution", "Date Comptable", "Statistique" };
	 ...
	 * Create the frame.
	 */
	public BatchEnvoiFichierCapiFrame() {
		...
		btnExcuter.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (radio_Virement.isSelected()) {
					...
					new Thread(new MoulinetteEnvoiFichierCapiTest(frame_Instance)).start();
		...
		radio_Virement = new JRadioButton("Virement CAPI");
