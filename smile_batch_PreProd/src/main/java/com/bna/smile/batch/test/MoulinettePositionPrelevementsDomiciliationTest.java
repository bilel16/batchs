...
public class MoulinettePositionPrelevementsDomiciliationTest implements Runnable {

	private static final Logger logger = Logger.getLogger(MoulinettePositionPrelevementsDomiciliationTest.class);
	private BatchFrame frame = null;
	private static final SimpleDateFormat formaterDate = new SimpleDateFormat("dd/MM/yyyy");

	public MoulinettePositionPrelevementsDomiciliationTest(BatchFrame frame) {
		super();
		this.frame = frame;
	}
	...
	public void run() {
		...
		if (listAgences != null && listAgences.size() > 0) {
			logger.info("*** Debut MoulinettePositionPrelevements ***");
		...
		frame.getEnd_exec_label().setText("La phase règlement est terminée avec succès");
		frame.getBtnExcuter().setEnabled(true);
		logger.info("*** FIN MoulinettePositionPrelevements ***");
