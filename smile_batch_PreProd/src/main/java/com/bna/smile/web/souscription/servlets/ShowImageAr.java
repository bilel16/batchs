...
public class ShowImageAr extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, 
        ...
        try {
           ...
           // BufferedImage biAr = signaturesContratCompteForm.setBufferedImageAr();
            BufferedImage biAr = (BufferedImage)  request.getSession().getAttribute("bufferedImageAr");  
            
            if (biAr != null) {
                    response.setContentType("image/jpeg");
                // send the image straight to the client
                out = response.getOutputStream();
                ImageIO.write(biAr, "JPEG", out);
