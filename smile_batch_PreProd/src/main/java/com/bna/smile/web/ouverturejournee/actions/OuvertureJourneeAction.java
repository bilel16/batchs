...
import com.bna.smile.model.domainecaisse.commande.GetSituationCaisseCentraleCmd;
...
public class OuvertureJourneeAction extends DispatchAction {
    ...
    public ActionForward initierPage(ActionMapping mapping, ActionForm form, 
        ...
        try {
            OuvertureJourneeForm ouvertureJourneeForm = 
                (OuvertureJourneeForm)form;
            ouvertureJourneeForm.clearForm();
            ...
            ouvertureJourneeForm.setDateJournee(DateHandler.dateJour());
            ouvertureJourneeForm.setCodStrcStrc(paramAgence.getCodStrcStrc().toString());
            ouvertureJourneeForm.setLibStrcStrc(structure.getLibStrcStrc());
            
            //-------------------------------------------------------//
            //- chercher la dérinière situation
            SituationCaisseCentraleVo situationCaisseCentraleVo = new SituationCaisseCentraleVo ();
            situationCaisseCentraleVo.setDateJournee(DateHandler.strToDate("23/01/2008"));
            situationCaisseCentraleVo.setCodeStructure(paramAgence.getCodStrcStrc());
            
            GetSituationCaisseCentraleCmd getSituationCaisseCentraleCmd = new GetSituationCaisseCentraleCmd();
            situationCaisseCentraleVo = (SituationCaisseCentraleVo) getSituationCaisseCentraleCmd.execute(situationCaisseCentraleVo);
            
            if (!situationCaisseCentraleVo.hasError()){
                if (situationCaisseCentraleVo.getCaisseDeviseCentrale()!= null){
                    ouvertureJourneeForm.setCodStatCca(situationCaisseCentraleVo.getCaisseCentrale().getCodStatCca());
                    ouvertureJourneeForm.setDatCloCca(DateHandler.dateToStr(situationCaisseCentraleVo.getCaisseCentrale().getDatCloCca())) ;
                    
                    if (situationCaisseCentraleVo.isExistCaisseDinars()){
                        ouvertureJourneeForm.setMontFinCdc(StrHandler.formatmnt(situationCaisseCentraleVo.getCaisseDinarsCentrale().getMontFinCdc()));
                        ouvertureJourneeForm.setMontInitCdc(StrHandler.formatmnt(situationCaisseCentraleVo.getCaisseDinarsCentrale().getMontInitCdc()));
                    }
                    if (situationCaisseCentraleVo.isExistCaisseDevise()){
                        ouvertureJourneeForm.setMontFinCdvc(StrHandler.formatmnt(situationCaisseCentraleVo.getCaisseDeviseCentrale().getMontFinCdvc()));
                        ouvertureJourneeForm.setMontInitCdvc(StrHandler.formatmnt(situationCaisseCentraleVo.getCaisseDeviseCentrale().getMontInitCdvc()));
                        
                    }
                    
                    if (situationCaisseCentraleVo.getListDetailCaisDevCen().size()>0){
                        ouvertureJourneeForm.setListDetailCaisseDeviseCentral(situationCaisseCentraleVo.getListDetailCaisDevCen());
                    }
                }
                
            }
            return mapping.findForward("pageOuvertureJournee");
        
        } catch(Exception e){
            com.oxia.fwk.core.Error erreur = new com.oxia.fwk.core.Error();
            StringBuffer text = 
                new StringBuffer("la transaction est Interrompu, une erreur dans OuvertureJourneeAction : ");
