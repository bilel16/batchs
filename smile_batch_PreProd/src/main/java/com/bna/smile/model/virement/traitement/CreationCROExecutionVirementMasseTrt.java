...
public class CreationCROExecutionVirementMasseTrt extends Traitement {
	...
	public CreationCROExecutionVirementMasseTrt() {
	}
	...
	public IValueObject perform(IValueObject vo) {
		...
		try {
			...
			operationMoyPayCompteDO.setLibObjOpOmp("EXECUTION VIREMENT DE MASSE " + motifEnvoi);
			...
			// Long montVirCcpt = Long.valueOf(globalVirement.getMntGvirGvir() - globalVirement.getMntRejetGvir());
			Long montVirCcpt = Long.valueOf(virementVo.getMontantGlobalByCB());

			// 06. getting provision
			Long montSoldThCcpt =
					(contratCptDO.getMontAutCcpt() + contratCptDO.getMontSoldCcpt() - contratCptDO.getMontBlocCcpt());
			...
			if (contratCptDO != null && (montVirCcpt > montSoldThCcpt)) {
				operationMoyPayCompteDO.setMontDinOmp(montSoldThCcpt);
				operationMoyPayCompteDO.setMontApreOmp(Long.valueOf("0"));
			} else {
			...
			tacheId.setCodTachTach(Constants.COD_TACH_EXECUTION_VIREMENT_MASSE);
			tacheId.setCodOperOper(Constants.COD_OPER_EXECUTION_VIREMENT_MASSE);
			tache.setOperation(operation);

			// 11. 1 Cas de Virement Emis
			if (Constants.COD_OPER_EXECUTION_VIREMENT_MASSE.equals(operation.getCodOperOper())) {
			...
			if (montant_commision != 0) {
				...
				// **** A verifier **** //
				nomencElemtCondition.setCodNecdNecd(Constants.COD_NECD_NECD_718 + "");
			...
			operationMoyPayCompteDO.setLibMotfOmp("EXECUTION VIREMENT MASSE");
			...
			objVirementVoCRO.setContratCpt(globalVirement.getContratCpt());
			...
			StringBuffer text = new StringBuffer("Erreur dans CreationCROExecutionVirementMasseTrt : ");
			text.append(e.toString());
			erreur.setCode("100");
			erreur.setDescription(text.toString());
			erreur.setKey("CreationCROExecutionVirementMasseTrt");
			virementVo.addError(erreur);
			logger.error("Erreur au niveau CreationCROExecutionVirementMasseTrt : ", e);
			virementVo.setMessageValidation("Probléme dans CreationCROExecutionVirementMasseTrt");
	...
	public void genCroText(ValueObject vo) {
		logger.info("starting Creation CRO DO 718");
		...
		this.setLibRefCro("EXECUTION VIREMENT DE MASSE " + motifRefCro);
		...
		cro.append(Long.valueOf(virementVo.getMontantGlobalByCB()) + ";");
