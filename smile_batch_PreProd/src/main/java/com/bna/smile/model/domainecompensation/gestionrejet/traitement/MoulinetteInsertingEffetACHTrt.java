...
/**
 * @author 5556
 * @since 16/04/2026 Refonte SNT - ACH
 **/
public class MoulinetteInsertingEffetACHTrt extends Traitement {
	...
	public IValueObject perform(IValueObject vo) {
		...
		try {
			...
			importFromBataBaseADT(compensationVo.getDateComptable(), ageBct, "41", "21, 22");

			/*******************************************************
			 * [LCN] [41-25]
			 *******************************************************/

			importFromBataBaseADTSort(compensationVo.getDateComptable(), ageBct, "41", "25");
			...
			importFromBataBaseADT(compensationVo.getDateComptable(), ageBct, "40", "21, 22");

			/**
			 * Valeur_40 25
			 */

			importFromBataBaseADTSort(compensationVo.getDateComptable(), ageBct, "40", "25");
			...
			logger.info("Importation DATA BASE ADT done.. avec erreur");

			throw new RuntimeException(ex);
		}

		logger.info("Importation DATA BASE ADT done..");

		return compensationVo;

	}

	public void importFromBataBaseADT(Date datComp, String ageBCT, String codVal, String codEnr) {
		...
		System.out.println(strc + "/" + codVal);

		List<EffetACHVo> listEffetsACH = new ArrayList<EffetACHVo>();

		listEffetsACH = compensationDAO.getListEffetsACHAgence(datComp, ageBCT, codVal, codEnr);

		long numberLine = listEffetsACH.size();

		if (listEffetsACH.size() > 0) {
			mntGlobaleFichier = Long.valueOf(listEffetsACH.get(0).getMntTot());
			nbrGlobalFichier = Long.valueOf(listEffetsACH.get(0).getNbrTot());
			fileName = listEffetsACH.get(0).getRefFic();
		}

		for (EffetACHVo effetACHVo : listEffetsACH) {

			/*if (line.startsWith("241") || line.startsWith("240") || line.startsWith("242")) {
				if (line.substring(26, 28).equals("00")) {
					createEffet(effetACHVo, fileName, numberLine, datComp, ageBCT);
				}

				if (!line.substring(26, 28).equals("00")) {
					createCompEffet(effetACHVo, fileName, numberLine, datComp, ageBCT);
				}
			}*/
		...
		logger.info("Importing Colonnes done:" + numberLine);
		...
		if (fileName.contains("-40-") && fileName.contains("-22-")) {
			...
			criteria1.add(expression.eq("codVal", Long.valueOf(40)));
			criteria1.add(expression.eq("codEnr", Long.valueOf(22)));
			criteria1.add(expression.eq("effetId.datOpe", compensationVo.getDateComptable()));
			criteria1.add(expression.isNull("codEtatEff"));
			criteria1.add(
					expression.eq("codAgeDes", StrHandler.lpad(compensationVo.getStructure().getCodBctStrc(), '0', 3)));

			list2221 = search.find(EffetRecuTmp.class, criteria1);
		}

		if (fileName.contains("-41-") && fileName.contains("-22-")) {
			criteria1.add(expression.eq("codVal", Long.valueOf(41)));
			criteria1.add(expression.eq("codEnr", Long.valueOf(22)));
			criteria1.add(expression.eq("effetId.datOpe", compensationVo.getDateComptable()));
			criteria1.add(expression.isNull("codEtatEff"));
			criteria1.add(
		...
		if (fileName.contains("-42-") && fileName.contains("-21-")) {
			criteria1.add(expression.eq("codVal", Long.valueOf(42)));
			criteria1.add(expression.eq("codEnr", Long.valueOf(21)));
			criteria1.add(expression.eq("effetId.datOpe", compensationVo.getDateComptable()));
			criteria1.add(expression.isNull("codEtatEff"));
			criteria1.add(
					expression.eq("codAgeDes", StrHandler.lpad(compensationVo.getStructure().getCodBctStrc(), '0', 3)));
	...
	public void importFromBataBaseADTSort(Date datComp, String ageBCT, String codVal, String codEnr)
		...
		logger.info("Importing Sort:" + fileName);
		...
		for (EffetACHVo effetACHVo : listEffetsACH) {
			/*if (line.startsWith("241") || line.startsWith("240") || line.startsWith("242"))
				if (line.substring(26, 28).equals("00") && line.length() > 211)
					createSort(effetACHVo, fileName, numberLine, datComp, ageBCT);*/
		...
		ICriteria criteria1 = search.createCriteria();
		expression = search.createExpression();
		criteria1.add(expression.eq("codVal", Long.valueOf(codVal)));
		criteria1.add(expression.eq("codEnr", Long.valueOf(codEnr)));
		criteria1.add(expression.eq("effetId.datOpe", datComp));
		criteria1.add(expression.isNull("codEtatEff"));
		criteria1.add(
				expression.eq("codAgeDes", StrHandler.lpad(compensationVo.getStructure().getCodBctStrc(), '0', 3)));
	...
	public void createEffet(EffetACHVo effetACHVo, String file, long compteur, Date dateCom, String ageBCT) {
		SimpleDateFormat formatDate = new SimpleDateFormat("yyyyMMdd");

		EffetRecuTmp tmp = new EffetRecuTmp();

		String strc = ageBCT;
		String codVal = "" + effetACHVo.getCodVal();

		logger.info(strc + "/" + codVal);

		Long codEnr = Long.valueOf(effetACHVo.getCodEnr());
		...
		if (codEnr.equals(Long.valueOf(22))) {
			...
			if (line.length() >= 375) {
				...
				effetId.setNumEff(effetACHVo.getNumEff());
				effetId.setDatOpe(effetACHVo.getDatCmp());
				...
				tmp.setCodAgeDes(effetACHVo.getCodAgeDes());
				tmp.setCodBanDes(effetACHVo.getCodBanDes());

				tmp.setCodBan(effetACHVo.getCodBan());
				tmp.setCodAge(effetACHVo.getCodAge());

				tmp.setNumLot(Long.valueOf(effetACHVo.getNumLot()));
				tmp.setCodEnr(Long.valueOf(effetACHVo.getCodEnr()));
				tmp.setCodDev(effetACHVo.getCodDev());
				tmp.setMntEff(Long.valueOf(effetACHVo.getMntEff()));
				tmp.setMntInt(Long.valueOf(effetACHVo.getMntInt()));
				tmp.setCodVal(Long.valueOf(effetACHVo.getCodVal()));

				Long mntProt = Long.valueOf(effetACHVo.getMntFra());
				if (!mntProt.equals(0L)) {
					tmp.setCodProt(1L);
				}
				tmp.setMntFra(mntProt);

				tmp.setRibTir(lpadS(effetACHVo.getRibTir(), "0", 20));
				tmp.setRibTirIni(lpadS(effetACHVo.getRibTirIni(), "0", 20));

				tmp.setRibBen(lpadS(effetACHVo.getRibBen(), "0", 20));
				tmp.setNomBen(effetACHVo.getNomBen());
				tmp.setNomTir(effetACHVo.getNomTir());

				try {
					tmp.setRefComTir(effetACHVo.getRefComTir());
				} catch (Exception ex) {

				}

				try {
					tmp.setCodAcc(Long.valueOf(effetACHVo.getCodAcc()));
				} catch (Exception ex) {
					tmp.setCodAcc(0L);
				}

				try {
					tmp.setCodEnd(Long.valueOf(effetACHVo.getCodEnd()));
				} catch (Exception ex) {
					tmp.setCodEnd(0L);
				}

				tmp.setDatEch(effetACHVo.getDatEch());
				tmp.setDatEchIni(effetACHVo.getDatEchIni());
				tmp.setDatCre(effetACHVo.getDatCre());
				tmp.setLieCre(effetACHVo.getLieCre());
				tmp.setRefComBen(effetACHVo.getRefComBen());

				try {
					tmp.setCodOrd(Long.valueOf(effetACHVo.getCodOrd()));
				} catch (Exception ex) {
					tmp.setCodOrd(0L);
				}

				try {
					tmp.setCodSit(Long.valueOf(effetACHVo.getCodSit()));
				} catch (Exception ex) {
					tmp.setCodSit(0L);
				}

				try {
					tmp.setCodNatCpt(Long.valueOf(effetACHVo.getCodNatCpt()));
				} catch (Exception ex) {
					tmp.setCodNatCpt(0L);
				}

				tmp.setDatCmp(effetACHVo.getDatCmp());

				tmp.setMotRej(effetACHVo.getMotRej());
				...
				try {
					tmp.setCodRisBct(effetACHVo.getCodRisBct());
				} catch (Exception ex) {
				...
				EffetId effetId = new EffetId();
				effetId.setNumEff(effetACHVo.getNumEff());
				effetId.setDatOpe(effetACHVo.getDatOpe());
				tmp.setEffetId(effetId);
			...
			if (tmp.getCodVal().equals(40L)) {
				if (!verifierNumEffet(tmp.getEffetId().getNumEff())) {
					String numeffet = tmp.getCodAgeDes() + tmp.getCodVal()
							+ tmp.getEffetId().getNumEff().substring(5, 12);
			...
			if (tmp.getRefComTir() == null || (tmp.getRefComTir() != null && !tmp.getRefComTir().startsWith("6"))) {
			...
			logger.info("Saving Effet:" + effetACHVo.getNumEff());
			...
			tmp.setCodEnr(Long.valueOf(effetACHVo.getCodEnr()));
			tmp.setCodDev(effetACHVo.getCodDev());
			tmp.setMntEff(effetACHVo.getMntEff() * 1000);

			tmp.setMntIntIni(effetACHVo.getMntInt());
			...
			if (tmp.getCodVal().equals(41L) && tmp.getCodEnr().equals(21L)) {
				if (tmp.getCodBan().equals(tmp.getCodBanDes()))
					emis = true;
				/*tmp.setImgRec(getImg(strc, tmp.getEffetId().getNumEff(), tmp.getCodBan(), "R",
						formatDate.format(dateCom), emis));
				tmp.setImgVes(getImg(strc, tmp.getEffetId().getNumEff(), tmp.getCodBan(), "V",
						formatDate.format(dateCom), emis));*/
			}
	...
	public void createSort(EffetACHVo effetACHVo, String file, long compteur, Date dateCom, String ageBCT) {
		...
		EffetId effetId = new EffetId();
		effetId.setNumEff(effetACHVo.getNumEff());
		effetId.setDatOpe(dateCom);
		tmp.setEffetId(effetId);
		tmp.setCodVal(Long.valueOf(effetACHVo.getCodVal()));

		tmp.setNumLot(effetACHVo.getNumLot());
		tmp.setCodEnr(Long.valueOf(effetACHVo.getCodEnr()));
		tmp.setCodDev(effetACHVo.getCodDev());
		tmp.setMntEff(effetACHVo.getMntEff() * 1000);

		tmp.setMntIntIni(effetACHVo.getMntInt());
		tmp.setMntInt(effetACHVo.getMntInt());
	...
	public void createCompEffet(EffetACHVo effetACHVo, String file, long compteur, Date dateCom, String ageBCT) {
		SimpleDateFormat formatDate = new SimpleDateFormat("yyyyMMdd");
	...
	public void ajouterFichierAvecMontant(String nomFichier, String codeStructure, Date datePECFichier,
			...
			long nbreTotalInter, long nbreTotalIntra, Long mntTotalIntra, Long mntGlobTotFichier,
			Long nbrGlobTotFichier) {
