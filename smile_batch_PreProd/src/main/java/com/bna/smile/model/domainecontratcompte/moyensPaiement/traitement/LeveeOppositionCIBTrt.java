 ...
 * 
 */
public class LeveeOppositionCIBTrt extends Traitement{
    public LeveeOppositionCIBTrt() {
    }
    ...
    public IValueObject perform(IValueObject vo) throws Exception{
        ...
        try {
            ...
            Date dateCirculation=oppositionMoyPaiementDAO.getDateCIBEnCirculation(paramOpposition.getNumCIB(),
            paramOpposition.getContratCpt().getContratCptId().getCodStrcStrc().toString(),paramOpposition.getContratCpt().getContratCptId().getCodPrdPrd().toString(),paramOpposition.getContratCpt().getContratCptId().getNumCcptCcpt().toString());
            ...
            //verifier si moyen paiement est déja en opposition
             ...
             if(listDernierEtatMoyenPaiement.size()>0){
                ...
                if(!codEtat.equals(Constants.COD_ETAT_OPMP_Opposition)){
                    ...
                    erreurEnOpposition.setDescription("CIB n'est pas mis en opposition");
                    paramOpposition.addError(erreurEnOpposition);
                    return paramOpposition;                   
                }
             }else{
    ...
    public String getNumeroTache(IValueObject vo){
        return Constants.COD_OPER_OPER_OPPOSITION_CIB_CLIENT.toString()+
            StrHandler.lpad(Constants.COD_TACH_TACH_LEVEE_CIB.toString(),'0',2);    
    }
