...
public class GestionEnvoiSMSPowerCardTrt extends Traitement {
	...
	public GestionEnvoiSMSPowerCardTrt() {
	}
	...
	public IValueObject perform(IValueObject vo) {
		...
		try {
			...
			numeroTel = envoiSMSVo.getNumTelephone();
			testMsg = envoiSMSVo.getPowerCardSMS().getTextSmsSal();
			...
			if (numeroTel != null && numeroTel.length() >= 8 && numeroTel.equalsIgnoreCase("00000000") == false) {
				...
				if (etatnumTelephone == true) {
					...
					this.sendPost();
					/************* update PowerCard **************/
					envoiSmsDAO.updateSMSPowerCard(envoiSMSVo.getPowerCardSMS());
			...
			e.printStackTrace();
			envoiSMSVo.setNumTelephone(envoiSMSVo.getNumTelephone());
			com.oxia.fwk.core.Error erreur = new com.oxia.fwk.core.Error();
			StringBuffer text = new StringBuffer("Erreur dans GestionEnvoiSMSPowerCardTrt : ");
			text.append(e.toString());
			erreur.setCode("100");
			erreur.setDescription(text.toString());
			erreur.setKey("GestionEnvoiSMSPowerCardTrt");
			envoiSMSVo.addError(erreur);
			logger.error("Erreur au niveau GestionEnvoiSMSPowerCardTrt : ", e);
			envoiSMSVo.setMessageValidation("Probléme dans GestionEnvoiSMSPowerCardTrt");
	...
	public boolean validateNumeroTelephone(String numero) {
		...
		try {
			if (!numero.trim().equals("") && numero.trim() != null) {
				...
				newNumber = (newNumber.replace(" ", "")).replace("+216", "");
	...
	private void sendPost() throws Exception {
		...
		String url = "http://10.1.8.12/wbmonitor/send/webapi/send_ack.php";
