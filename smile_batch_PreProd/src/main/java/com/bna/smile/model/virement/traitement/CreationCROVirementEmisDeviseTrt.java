...
public class CreationCROVirementEmisDeviseTrt extends Traitement {
	...
	long montant_tva = 0;
	long montant_commision_Devise = 0;
	long montant_tva_Devise = 0;
	long montantGlobalVirement = 0;
	long montantDeviseVirement = 0;
	...
	public CreationCROVirementEmisDeviseTrt() {
	}
	...
	public IValueObject perform(IValueObject vo) {
		...
		try {
			...
			montantGlobalVirement = globalVirement.getMntGvirGvir();
			montantDeviseVirement = globalVirement.getMntDevGvir();
			operation.setCodOperOper(Constants.COD_OPER_VIREMENT_EMIS_DEVISE_MEME_AG);
			...
			operationMoyPayCompteDO.setLibObjOpOmp("VIREMENT EMIS MÊME DEVISES MÊME DEVISES ");
			...
			Long montVirDeviseCcpt = new Long(montantDeviseVirement);

			// 05. setting devise et montant
			Devise devise = new Devise();
			devise.setCodDevDev(contratCptDO.getDevise().getCodDevDev());

			// 06. Virement en dinar

			if (contratCptDO != null) {

				operationMoyPayCompteDO.setMontDinOmp(montVirCcpt);
				operationMoyPayCompteDO.setMontApreOmp(
						contratCptDO.getMontSoldCcpt() - (montVirCcpt + montant_commision + montant_tva));

				operationMoyPayCompteDO.setMontDevOmp(montVirDeviseCcpt);
				operationMoyPayCompteDO.setMontDevApreOmp(contratCptDO.getMontSdevCcpt()
						- (montVirDeviseCcpt + montant_commision_Devise + montant_tva_Devise));
				operationMoyPayCompteDO.setMontCvalOmp(montVirCcpt);

			}

			operationMoyPayCompteDO.setMontSoldCcpt(contratCptDO.getMontSoldCcpt());
			operationMoyPayCompteDO.setMontSdevCcpt(contratCptDO.getMontSdevCcpt());
			...
			if (globalVirement.getTypePouvGvir().equals(Constants.COD_TYPE_POUVOIR_TITULAIRE)) {
				...
				typePieceDemandeur.setCodTpceTpce(
						Long.valueOf(contratCptDO.getClient().getPersonne().getTypePiece().getCodTpceTpce()));
				...
				if (globalVirement.getMandantPersVirements() != null
					...
					try {
						...
						TypePiece typePieceDemandeur = new TypePiece();
						typePieceDemandeur.setCodTpceTpce(
								Long.valueOf(mandatPersonne.getPersonne().getTypePiece().getCodTpceTpce()));
						operationMoyPayCompteDO.setTypePieceDemandeur(typePieceDemandeur);
				...
				if (globalVirement.getPersCotitVirements() != null
						&& globalVirement.getPersCotitVirements().size() > 0) {
					try {
				...
				try {
					...
					if (list != null && list.size() > 0) {
						...
						if (traceOperVirement.getPersonneDem() != null
							...
							TypePiece typePieceDemandeur = new TypePiece();
							typePieceDemandeur.setCodTpceTpce(
									Long.valueOf(traceOperVirement.getPersonneDem().getTypePiece().getCodTpceTpce()));
							operationMoyPayCompteDO.setTypePieceDemandeur(typePieceDemandeur);
			...
			tacheId.setCodOperOper(Constants.COD_OPER_VIREMENT_EMIS_DEVISE_MEME_AG);
			tache.setOperation(operation);

			// 11. 1 Cas de Virement Emis
			if (Constants.COD_OPER_VIREMENT_EMIS_DEVISE_MEME_AG.equals(operation.getCodOperOper())) {
			...
			operationMoyPayCompteDO.setMontTvadOmp(montant_tva_Devise);
			 ...
			 * 
			 * // **** A verifier **** // nomencElemtCondition.setCodNecdNecd(Constants.COD_NECD_NECD_715 + "");
			 * 
			 * detailOmpCommission.setNomencElemtCondition(nomencElemtCondition);
			 * detailOmpCommission.setCodTypDomp(Constants.COD_TYPE_COMMISSION); detailOmpCommission.setMontValDomp(new
			 * Long(montant_commision)); detailOmpCommission.setDatValDomp(DateHandler.strToDate(dateValueComRecu));
			 * detailOmpCommission.setOperationMoyPay(operationMoyPayCompteDO); setDetOpm.add(detailOmpCommission);
			 * 
			...
			operationMoyPayCompteDO.setLibMotfOmp("VIREMENT DEVISE");
			...
			if (etatClientTaxable == true) {
				...
				montantAjouterAuSoldeDevise = montVirDeviseCcpt + montant_commision_Devise + montant_tva_Devise;
			} else {
				montantAjouterAuSolde = montVirCcpt + montant_commision;
				montantAjouterAuSoldeDevise = montVirDeviseCcpt + montant_commision_Devise;
			}
			...
			contratCptDO = (ContratCpt) updateSoldTrt.exec(contratCptSold);
			...
			StringBuffer text = new StringBuffer("Erreur dans CreationCROVirementEmisDeviseTrt : ");
			text.append(e.toString());
			erreur.setCode("100");
			erreur.setDescription(text.toString());
			erreur.setKey("CreationCROVirementEmisDeviseTrt");
			virementVo.addError(erreur);
			logger.error("Erreur au niveau CreationCROVirementEmisDeviseTrt : ", e);
			virementVo.setMessageValidation("Probléme dans CreationCROVirementEmisDeviseTrt");
	...
	public void genCroText(ValueObject vo) {
		logger.info("starting Creation CRO DO 2086");
		...
		this.setLibRefCro("VIREMENT EMIS MÊME DEVISES MÊME DEVISES");
		...
		cro.append(globalVirement.getDevise().getCodDevDev() + ";");

		// COUR_FIX_CPT

		cro.append("COUR_FIX_CPT=");
		cro.append(globalVirement.getCourFixGvir() + ";");
		...
		cro.append(globalVirement.getMntGvirGvir() + "_$$_TND;");

		// MNT_GLO_DEV_REM
		cro.append("MNT_GLO_DEV_REM=");
		if (devise != null && devise.getLibSiglDev() != null) {
			cro.append(globalVirement.getMntDevGvir() + "_$$_" + devise.getLibSiglDev() + ";");
		} else {
			cro.append(globalVirement.getMntDevGvir() + ";");
		}
		...
		// Montant Commission
		cro.append("267=");
		cro.append(montant_commision + "_$$_TND;");
