...
import com.bna.commun.model.ReservationCheque;
...
public class DeReservationChequeTrt extends Traitement {
	...
	public DeReservationChequeTrt() {
	}

	public IValueObject perform(IValueObject vo) {
		logger.info("Cro d'operation Dereservation montant Cheque");
		...
		Long sommeReservation = 0L;
		Operation oper = new Operation();
		oper.setCodOperOper(Constants.COD_OPERATION_DERESERVATION_CHEQUE);
		...
		try {
			...
			criteria.add(expression.eq("numCcptCcpt", contratCptTireur.getContratCptId().getNumCcptCcpt()));
			criteria.add(expression.eq("codPrdPrd", contratCptTireur.getContratCptId().getCodPrdPrd()));
			criteria.add(expression.eq("codStrcStrc", contratCptTireur.getContratCptId().getCodStrcStrc()));
			criteria.add(expression.eq("codEtatResrv", "R"));
			criteria.add(expression.ge("datExpireResrv", reglementChequeVo.getDateComptable()));
			criteria.add(expression.eq("numChqChq", reglementChequeVo.getCheque().getChequeId().getNumChqChq()));

			List<ReservationCheque> listeReservationCheque = searchEngine.find(ReservationCheque.class, criteria);
			logger.info("List ReservationCheque to Update:" + listeReservationCheque.size());
			for (int i = 0; i < listeReservationCheque.size(); i++) {
				ReservationCheque reservationCheque = listeReservationCheque.get(i);

				/** mettre datFin pour tout les reservation relatives aux cheque en question */
				reservationCheque.setDatFinvResrv(reglementChequeVo.getDateComptable());
				reservationCheque.setCodEtatResrv("D");
				crudService.update(reservationCheque);

				/**
				 * le calcul de la somme blocage ne se fait que pour le montant chéque ;; c'est la somme avec laquel on
				 * alimente le compte client
				 */

				sommeReservation += reservationCheque.getMntResrvResrv();

			}

			/**
			 * on n'insere operation moy pay que si le compte client est modifié , on alimente le compte client par la
			 * somme reservation pour CHEQUE
			 */
			if (UtilCtr.isContratValide(contratCptTireur)) {
				...
				operationMoyPay.setLibObjOpOmp("DERESERVATION MONTANT CHEQUE N° "
				...
				operationMoyPay.setMontDinOmp(sommeReservation);
				operationMoyPay.setMontApreOmp(contratCptTireur.getMontSoldCcpt() + sommeReservation);
				...
				operationMoyPay.setCodRefcOmp(reglementChequeVo.getCheque().getChequeId().getNumChqChq() + "");
				...
				tacheId.setCodOperOper(Constants.COD_OPERATION_DERESERVATION_CHEQUE);
				...
				operationMoyPay.setLibMotfOmp("DERESERVATION MONTANT CHEQUE N° "
						+ reglementChequeVo.getCheque().getChequeId().getNumChqChq());
				...
				mouvementCompensation.setMntOpeMvtc(sommeReservation);
				...
				contratCptSold.setSolde(sommeReservation);
			...
			StringBuffer text = new StringBuffer("Erreur dans DeReservationChequeTrt : ");
			text.append(e.toString());
			erreur.setCode("100");
			erreur.setDescription(text.toString());
			erreur.setKey("DeReservationChequeTrt");
			reglementChequeVo.addError(erreur);
			logger.error("Erreur au niveau DeReservationChequeTrt : ", e);
	...
	public void genCroText(ValueObject vo) {
		...
		operation.setCodOperOper(Constants.COD_OPERATION_DERESERVATION_CHEQUE);
		...
		this.setLibRefCro("DERESERVATION MONTANT CHEQUE");
		...
		// *** Mnt Cheque ****//
		cro.append("MNT_CHQ_CLT="+ reglementChequeVo.getCheque().getMntChqChq().longValue()+";");
		long mntRESERVATION = Long.valueOf(0);
		if (reglementChequeVo.getSommeReserve() != null)
			mntRESERVATION = reglementChequeVo.getSommeReserve().longValue();
		// *** Montant DeReservation ****//
		cro.append("MNT_RESV_CHQ_CLT=");
		cro.append(mntRESERVATION + ";");
		...
		cro.append("ETAT_CPT=");
		if (contratCptTireur.getCodEtatCcpt().equals(Constants.COD_VALIDATION)) {
			cro.append("0;");
		} else if (contratCptTireur.getCodEtatCcpt().equals(Constants.COD_ETAT_CPT_TCONTENTIEU)
				|| contratCptTireur.getCodEtatCcpt().equals(Constants.COD_ETAT_CPT_RESILIE)) {
			cro.append("6;");
		}
		...
		logger.info("Cro Dereservation Montant Cheque generer avec succés");
	}
