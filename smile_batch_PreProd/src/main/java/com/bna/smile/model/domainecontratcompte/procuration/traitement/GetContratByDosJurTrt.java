...
public class GetContratByDosJurTrt extends Traitement {
    public GetContratByDosJurTrt() {
    }
    ...
    public IValueObject perform(IValueObject vo) {
        ...
        try {
            ...
            if (personne.getNumSeqPers() != null) {
                ...
                client = 
                        (Client)searchEngine.get(Client.class, personne.getNumSeqPers());
                ...
                if (personneStrc.getCodStrcStrc() != 
                    null) { /// cas agence ou direction regionale
                    ...
                    if (structure.getTypeStructure().getCodTstrTstr().equals(new Long(2))) { /// cas DR
                        ...
                        criteriaStrc.add(expression.eq("structure.codStrcStrc", 
                                                       personneStrc.getCodStrcStrc()));
                        List listStructures = 
                            searchEngine.find(Structure.class, criteriaStrc);
                        for (int i = 0; i < listStructures.size(); i++) {
                            Structure st = (Structure)listStructures.get(i);
                            ICriteria criteriaSt = 
                                searchEngine.createCriteria();
                            criteriaSt.add(expression.eq("contratCptId.codStrcStrc", 
                                                         st.getCodStrcStrc()));
                            criteriaSt.add(expression.eq("client.numSeqPers", 
                                                         personne.getNumSeqPers()));
                            criteriaSt.add(expression.eq("codEtatCcpt", "V"));
                            List lContrat = 
                                searchEngine.find(ContratCpt.class, criteriaSt);
                        ...
                        criteriaCpt.add(expression.eq("contratCptId.codStrcStrc", 
                                                      personneStrc.getCodStrcStrc()));
                        listCptValid = 
                                searchEngine.find(ContratCpt.class, criteriaCpt);
                    }
                } else
                    listCptValid = 
                            searchEngine.find(ContratCpt.class, criteriaCpt);
            ...
            StringBuffer text = 
                new StringBuffer("Erreur dans GetPersonneCptTrt : ");
