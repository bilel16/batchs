...
public class CreationCROPositionPrelevementsRecusTrt extends Traitement {
	...
	public CreationCROPositionPrelevementsRecusTrt() {
	}
	...
	public IValueObject perform(IValueObject vo) {
		...
		try {
			...
			if (contratCptTireur.getClient() != null && contratCptTireur.getClient().getNumCtxClt() != null
					&& contratCptTireur.getClient().getNumCtxClt() != 0) {

				numCtxClt = contratCptTireur.getClient().getNumCtxClt();
				structureCtx = contratCptTireur.getClient().getStructure();
			}

			// /-------------------- Condition de Banque Tireur ---------------------------///
			boolean etatBenifMemeAgence = false;
			boolean etatBenifAutreAgence = false;
			boolean etatBenifAutreBanque = false;
			List<String> palier = new ArrayList<String>();
			String ribBenif = detailsPrelevements.getDetailsPrelevementsId().getRibBenPrl();
			if (ribBenif != null && ribBenif.length() == 20) {

				if (ribBenif.substring(0, 2).equals("03")) {
					String codeAgenceBenif = ribBenif.substring(5, 8);
					if (Long.valueOf(codeAgenceBenif).longValue() == contratCptTireur.getContratCptId().getCodStrcStrc()
							.longValue()) {
						etatBenifMemeAgence = true;
					} else {
						etatBenifAutreAgence = true;

					}

				} else {
					etatBenifAutreBanque = true;
				}
			...
			if (etatBenifMemeAgence == true) {
				palier.add("41");
			} else if (etatBenifAutreAgence == true) {
				palier.add("42");
			} else if (etatBenifAutreBanque == true) {
				palier.add("43");
			}
			...
			traitementConditionBanque.setCodStrcStrc(contratCptTireur.getContratCptId().getCodStrcStrc().toString());
			traitementConditionBanque.setCodPrdCpt(contratCptTireur.getContratCptId().getCodPrdPrd().toString());
			traitementConditionBanque.setNumCcptCcpt(contratCptTireur.getContratCptId().getNumCcptCcpt().toString());
			traitementConditionBanque.setDateReference(
					DateHandler.dateToStr(detailsPrelevements.getDetailsPrelevementsId().getDatOpePrl()));
			traitementConditionBanque.setMontant(detailsPrelevements.getDetailsPrelevementsId().getMntPrlPrl() + "");
			if (palier.size() > 0) {
				traitementConditionBanque.setPalierChar(new ArrayList<String>(palier));
			}
			...
			if (contratCptTireur.getClient().getPersonne() != null
				...
				traitementConditionBanque.setCodTpceTpce(
						contratCptTireur.getClient().getPersonne().getTypePiece().getCodTpceTpce() + "");
			...
			operationMoyPay.setLibObjOpOmp("REGLEMENT DES PRELEVEMENTS");
			...
			Long montVirCcpt = detailsPrelevements.getDetailsPrelevementsId().getMntPrlPrl();
			...
			try {
				if (contratCptTireur.getDatEautCcpt() != null
					...
					montSoldThCcpt = (contratCptTireur.getMontAutCcpt() + contratCptTireur.getMontSoldCcpt()
							- contratCptTireur.getMontBlocCcpt());
				} else {
			...
			if (contratCptTireur != null) {
				...
				operationMoyPay.setMontApreOmp(
						contratCptTireur.getMontSoldCcpt() - (montVirCcpt + montant_commissionRecue + montant_tva));
			}
			...
			if (detailsPrelevements.getLibPrlPrl() != null && detailsPrelevements.getLibPrlPrl().length() >= 30) {
				operationMoyPay.setCodRefbOmp(detailsPrelevements.getLibPrlPrl().substring(0, 29));
			} else if (detailsPrelevements.getLibPrlPrl() != null && detailsPrelevements.getLibPrlPrl().length() < 30) {
			...
			typePieceDemandeur.setCodTpceTpce(
					Long.valueOf(contratCptTireur.getClient().getPersonne().getTypePiece().getCodTpceTpce()));
			...
			oper.setCodOperOper(Constants.COD_OPER_REGLEMENT_PRELEVEMENT);
			Tache tache = new Tache();
			tache.setOperation(oper);
			TacheId tacheId = new TacheId();
			tacheId.setCodOperOper(Constants.COD_OPER_REGLEMENT_PRELEVEMENT);
			tacheId.setCodTachTach(Constants.COD_TACH_REGLEMENT_PRELEV_RECU);
			...
			if (montant_commissionRecue != 0) {
				...
				// **** A verifier **** //
				nomencElemtCondition.setCodNecdNecd(Constants.COD_NECD_NECD_818 + "");
			...
			operationMoyPay.setLibMotfOmp("REGLEMENT DES PRELEVEMENTS");
			...
			contratCptSold.setSolde(detailsPrelevements.getDetailsPrelevementsId().getMntPrlPrl().longValue()
					+ montant_commissionRecue + montant_tva);
			...
			StringBuffer text = new StringBuffer("Erreur dans CreationCROPositionPrelevementsRecusTrt : ");
			text.append(e.toString());
			erreur.setCode("100");
			erreur.setDescription(text.toString());
			erreur.setKey("CreationCROPositionPrelevementsRecusTrt");
			prelevementVo.addError(erreur);
			logger.error("Erreur au niveau CreationCROPositionPrelevementsRecusTrt : ", e);
	...
	public void genCroText(ValueObject vo) {
		logger.info("starting CreationCROPositionPrelevementsRecusTrt method ");
		...
		this.setLibRefCro("REGLEMENT DES PRELEVEMENTS");
		...
		this.setCodTachTach(Constants.COD_TACH_REGLEMENT_PRELEV_RECU);
		...
		cro.append("MNT_PRL_PRL=");
		cro.append(detailsPrelevements.getDetailsPrelevementsId().getMntPrlPrl() + ";");

		// *** Montant commission sur prélèvement ****//
		cro.append("36=");
		cro.append(montant_commissionRecue + ";");

		// *** Montant TVA sur prélèvement ****//
		cro.append("MNT_TVA_PRL=");
		...
		cro.append(detailsPrelevements.getDetailsPrelevementsId().getRibBenPrl() + ";");
