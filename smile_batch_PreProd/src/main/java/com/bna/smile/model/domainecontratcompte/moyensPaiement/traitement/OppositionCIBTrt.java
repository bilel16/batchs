 ...
 * 
 */
public class OppositionCIBTrt  extends Traitement{
    public OppositionCIBTrt() {
    }
    ...
    public IValueObject perform(IValueObject vo) throws Exception{
        ...
        try {
                ...
                String numCIB = paramOpposition.getNumCIB(); 
                ...
                dateCirculation=oppositionMoyPaiementDAO.getDateCIBEnCirculation(numCIB,paramOpposition.getContratCpt().getContratCptId().getCodStrcStrc().toString(),paramOpposition.getContratCpt().getContratCptId().getCodPrdPrd().toString(),paramOpposition.getContratCpt().getContratCptId().getNumCcptCcpt().toString());
                if(dateCirculation == null){
                    com.oxia.fwk.core.Error erreurEnOpposition = 
                        new com.oxia.fwk.core.Error();
                    erreurEnOpposition.setCode("MoyPayEnCirculationCIB");
                    erreurEnOpposition.setDescription("CIB n'est pas en circulation pour ce compte" );
                    paramOpposition.addError(erreurEnOpposition);
                    return paramOpposition;      
                }
                 
                //verifier si moyen paiement est déja en opposition
                 listDernierEtatMoyenPaiement=oppositionMoyPaiementDAO.getDernierEtatMoyPaiement(Constants.COD_MOYP_TMOY_CIB.toString(),numCIB,paramOpposition.getContratCpt().getContratCptId().getCodStrcStrc().toString(),paramOpposition.getContratCpt().getContratCptId().getCodPrdPrd().toString(),paramOpposition.getContratCpt().getContratCptId().getNumCcptCcpt().toString());
                 if(listDernierEtatMoyenPaiement.size()>0){
                    ...
                    if(codEtat.equals(Constants.COD_ETAT_OPMP_Opposition)){
                        ...
                        erreurEnOpposition.setDescription("CIB est déja mis en opposition le "+DateHandler.dateToStr(dateOperation));
                        paramOpposition.addError(erreurEnOpposition);
                        return paramOpposition;                   
                    }
                 }
                ...
                tacheId.setCodOperOper(Constants.COD_OPER_OPER_OPPOSITION_CIB_CLIENT);
                tacheId.setCodTachTach(Constants.COD_TACH_TACH_OPPOSITION_CIB_CLIENT);
                ...
                oppositionMoyenPaiementId.setCodMoypTmoy(Constants.COD_MOYP_TMOY_CIB);
                oppositionMoyenPaiementId.setNumMoypOpmp(numCIB);
            ...
            erreur.setDescription("OppositionCIBTrt " + e.getMessage());
    ...
    public String getNumeroTache(IValueObject vo){
        return Constants.COD_OPER_OPER_OPPOSITION_CIB_CLIENT.toString()+
            StrHandler.lpad(Constants.COD_TACH_TACH_OPPOSITION_CIB_CLIENT.toString(),'0',2);
