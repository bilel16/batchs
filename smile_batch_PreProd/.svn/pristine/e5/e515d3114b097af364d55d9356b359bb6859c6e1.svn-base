...
public class CreationCROPositionVirementTrt extends Traitement {
	...
	public CreationCROPositionVirementTrt() {
	}
	...
	public IValueObject perform(IValueObject vo) {
		...
		try {
			...
			if (boolValiderBenif == true) {
				...
				contratCptDO = (ContratCpt) searchEngine.get(ContratCpt.class,
						globalVirement.getContratCpt().getContratCptId());

				if (contratCptBenif != null && contratCptBenif.getContratCptId() != null
						&& (contratCptBenif.getContratCptId().getCodPrdPrd().equals(Constants.COD_PRD_PRD_EPS)
								|| contratCptBenif.getContratCptId().getCodPrdPrd()
				...
				} else if (contratCptBenif != null && contratCptBenif.getContratCptId() != null) {
				...
				/***************** CAS Vir Provenant de 177 *************/

				if (contratCptDO != null && contratCptDO.getClient() != null
						&& contratCptDO.getClient().getNumSeqPers() != null) {
					if (contratCptDO.getClient().getNumSeqPers().equals(contratCptBenif.getClient().getNumSeqPers())
							&& contratCptDO.getContratCptId().getCodPrdPrd().equals(Constants.COD_PRD_PRD_PEE)
							&& (contratCptBenif.getContratCptId().getCodPrdPrd().equals(101L)
									|| contratCptBenif.getContratCptId().getCodPrdPrd().equals(112L))) {
						montant_commissionRecue = 0l;
						montant_tva = 0l;
				...
				if (detailVirementObj.getGlobalVirement().getContratCpt().getClient() != null && detailVirementObj
						.getGlobalVirement().getContratCpt().getClient().getPersonne().getNomRsPers() != null) {

					String relation = detailVirementObj.getGlobalVirement().getContratCpt().getClient().getPersonne()
							.getNomRsPers();
					if (relation.length() > 60) {
						relation = relation.substring(0, 59);
					}
					operationMoyPay.setNomNomdOmp(relation);
				}
				...
				if (detailVirementObj.getNomBenDetv() != null && detailVirementObj.getNomBenDetv().length() > 60) {
					operationMoyPay.setNomNombOmp(detailVirementObj.getNomBenDetv().substring(0, 59));
				} else {
				...
				detailsOperationVirement
						.setCodStrcStrc(detailVirementObj.getGlobalVirement().getStructure().getCodStrcStrc());
				...
				if (globalVirement != null && globalVirement.getNumSeqGvir() != null
					...
					if (personne != null && personne.getNumSeqPers() != null) {
						...
						criteriaHer.add(expressionHer.eq("persClientId.numSeqCli",
								globalVirement.getContratCpt().getClient().getNumSeqPers()));
	...
	public void genCroText(ValueObject vo) {
		...
		if (virementVo.getNumCtxClt() == 0) {
			if (etatBenifTaxable == true) {
				MNT_BRUT_COM = montant_commissionRecue + montant_tva;
			} else {
				MNT_BRUT_COM = montant_commissionRecue;
			}
		} else {
			MNT_BRUT_COM = 0L;
		}
