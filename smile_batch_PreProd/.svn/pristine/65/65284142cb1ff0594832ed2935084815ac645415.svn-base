...
public class Cro841ReceptionBCTrt extends Traitement {
	...
	public Cro841ReceptionBCTrt() {
	}

	public IValueObject perform(IValueObject vo) {
		logger.info("Cro d'operation recepetion BC");
		...
		try {
			...
			StringBuffer text = new StringBuffer("Erreur dans Cro841ReceptionBCTrt : ");
			text.append(e.toString());
			erreur.setCode("100");
			erreur.setDescription(text.toString());
			erreur.setKey("Cro841ReceptionBCTrt");
			compensationEffetVo.addError(erreur);
			logger.error("Erreur au niveau Cro841ReceptionBCTrt : ", e);
	...
	public void genCroText(ValueObject vo) {
		try {
			...
			operation.setCodOperOper(Constants.COD_OPER_RECEP_COMP_BC);
			...
			String refIntSg =
					StrHandler.lpad("" + compensationEffetVo.getStructure().getCodStrcStrc(), '0', 3) + quantieme
							+ "2EF";
			this.setCodRefInter(refIntSg);
			this.setNumRefCro(Long.valueOf("9999"));
			this.setLibRefCro("Réception presentation Effet BC");
