...
public abstract class AbstractValidator {
		...
		if (numPiece != null && !"".equals(numPiece.trim())) {
			if (Constants.COD_CIN.toString().equals(typePiece)) {
				if (!numPiece.matches("[0-9]{8}")) {
					//GenerateMessage.creatMessage("errors.cin.invalid_number", null, componentName);
					return false;
				} else {
					if (numPiece.matches("1{8}") || numPiece.matches("2{8}") || numPiece.matches("3{8}")
						...
						//GenerateMessage.creatMessage("errors.cin.invalid", null, componentName);
						return false;
					}
				}
			} else {
				if (Constants.COD_CSEJ.toString().equals(typePiece) || Constants.COD_PASS.toString().equals(typePiece)) {
	...
	public static String validAndFormatMontantAndGetMessage(String montant, String componentName) {
		...
		if (res == null) {
			//GenerateMessage.creatMessage("errors.amount.invalid", null, componentName);
		}
		return res;
	}
