...
public class MoulinetteEnvoiFichierCapiTest implements Runnable {

	private static final Log logger = LogFactory.getLog(MoulinetteEnvoiFichierCapiTest.class);
	private BatchEnvoiFichierCapiFrame frame = null;
	private static final SimpleDateFormat formaterDate = new SimpleDateFormat("dd/MM/yyyy");

	/***********************************/
	public MoulinetteEnvoiFichierCapiTest() {
		super();

	}

	public MoulinetteEnvoiFichierCapiTest(BatchEnvoiFichierCapiFrame frame) {
		super();
		this.frame = frame;
	}

	public void setFrame(BatchEnvoiFichierCapiFrame frame) {
		this.frame = frame;
	}

	public BatchEnvoiFichierCapiFrame getFrame() {
		return frame;
	}
	...
	public void run() {
		...
		batchMetier.setCodBatBmet(Constants.COD_BATCH_ENVOI_FICHIER_CAPI);
		...
		if (batchVo.getListeTraceBatch() != null && batchVo.getListeTraceBatch().size() > 0) {
			frame.getEnd_exec_label().setText("Une autre moulinette capi est en cours d'exécution");
			...
			batchMetierVirement.setCodBatBmet(Constants.COD_BATCH_ENVOI_FICHIER_CAPI);
...
//				if (listAgences != null && listAgences.size() > 0) {
					logger.info("*** Debut Moulinette capi ***");
...
//						infoVo.setStructure("" + agence.getCodStrcStrc());
						...
						infoVo.setDateComptable(formaterDate.format(new Date()));
						frame.addOrUpdateEtat(infoVo);

						// ********Batch *************//
						MoulinetteEnvoiFichierCapiTest2 moulinette =
								new MoulinetteEnvoiFichierCapiTest2();
...
//				}
				...
				frame.getEnd_exec_label().setText("Moulinette Envoie Fichier CAPI terminée avec succès");
				frame.getBtnExcuter().setEnabled(true);
