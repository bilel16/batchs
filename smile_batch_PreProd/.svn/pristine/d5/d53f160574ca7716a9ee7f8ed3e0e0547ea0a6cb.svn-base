...
public class GetSituationMensuelleIntByStrcTrt extends Traitement{
    public GetSituationMensuelleIntByStrcTrt() {
    }   
    ...
    List situationMensuelleDao = plcDao.getSituationMensuelleInteret(codeStructre,dateComptable);
        ...
        if(situationMensuelleDao!=null && situationMensuelleDao.size()>0) {
            ...
            for (Iterator it1 = situationMensuelleDao.iterator(); it1.hasNext(); ){
                ...
                if (sitMens.getValue(0) != null) {
                   ...
                   recap.setMntIrcServi(StrHandler.formatmnt(Long.valueOf(sitMens.getValue(0).toString()).doubleValue()));

                }
                if (sitMens.getValue(1) != null) {

                  recap.setMntIntertServiBrut(StrHandler.formatmnt(Long.valueOf(sitMens.getValue(1).toString()).doubleValue()));

                }
                if (sitMens.getValue(2) != null) {

                   recap.setMntIntertServi(StrHandler.formatmnt(Long.valueOf(sitMens.getValue(2).toString()).doubleValue()));

                }
                if (sitMens.getValue(3) != null) {
                    recap.setLibProduit(sitMens.getValue(3).toString());
                }
                if (sitMens.getValue(4) != null) {
                    recap.setCodStrcStrc(Long.valueOf(sitMens.getValue(4).toString()));
                }
