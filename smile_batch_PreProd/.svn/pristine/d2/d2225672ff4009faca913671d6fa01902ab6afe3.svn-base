...
public class MAJNSIActelFromOmpTrt extends Traitement {
	...
	public IValueObject perform(IValueObject vo) {
		...
		this.mainFrame = mAJNSIVo.getMainFrameActel();
		...
		try {
			...
			for (int i = 0; i < listAgencesMAJNSIStrcBtch.size(); i++) {
				...
				SwingInfoVo infoVo = new SwingInfoVo();
				infoVo.setStructure("" + journeeStructureBatch.getJourneeStructureBatchId().get_codStrcStrc());
				infoVo.setEtat(Constants.STATUT_EN_TERMINE);
				infoVo.setDateComptable(
						formatDate.format(journeeStructureBatch.getJourneeStructureBatchId().get_datJrnJrn()));
