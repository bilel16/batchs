...
public class ExecutionDoVirMasseSalaireTrt extends Traitement {
	...
	public ExecutionDoVirMasseSalaireTrt() {
	}
	...
	public IValueObject perform(IValueObject vo) {
		...
		long nbreVirementAutreAgence_Banque = 0;
		...
		try {
			...
			criteria.add(expression.gt("mntDetvDetv", 0L));
			...
			if (list_Details != null && list_Details.size() > 0) {
				...
				for (DetailVirement detailVirement : list_Details) {
					...
					if (codeBanque.equals("03")) {
						...
						if (new Long(detailVirement.getRibBenDetv().substring(5, 8)).longValue() == globalVirement
							...
							mntVirementAutreAgence_Banque += detailVirement.getMntDetvDetv();
							nbreVirementAutreAgence_Banque++;
				...
				VirementVo virementVoCBAutreAgence_Banque = new VirementVo();

				VirementVo virementVoCBAutreBanqueSGMT = new VirementVo();
				ArrayList<String> palierCharMemeAgence = new ArrayList<String>();
				ArrayList<String> palierCharAutreAgence_Banque = new ArrayList<String>();
				...
				if (nbreVirementAutreAgence_Banque > 0) {

					palierCharAutreAgence_Banque.add("43");

					virementVoCBAutreAgence_Banque.setContratCpt(globalVirement.getContratCpt());
					virementVoCBAutreAgence_Banque.setOperation(operationCB);
					virementVoCBAutreAgence_Banque.setMontant_virement(new Long("10000"));
					virementVoCBAutreAgence_Banque.setGlobalVirement(globalVirement);
					virementVoCBAutreAgence_Banque.setDateComptableAgence(dateComptable);
					virementVoCBAutreAgence_Banque.setListePalierCaractere(palierCharAutreAgence_Banque);
					virementVoCBAutreAgence_Banque = getConditionDeBanque(virementVoCBAutreAgence_Banque);
					dateValeurAutreAgence_Banque = virementVoCBAutreAgence_Banque.getValueDateRecue();
					dateValeurCommission = virementVoCBAutreAgence_Banque.getValueDateComRecue();
				}
				...
				mntCommissionVirementMemeAgence = (nbreVirementMemeAgence * mntCommissionVirementPetitMnt);

				mntCommissionVirementAutreAgence_banque =
						(nbreVirementAutreAgence_Banque * mntCommissionVirementPetitMnt);

				mntCommissionVirementAutreBanqueSGMT = (nbreVirementAutreBanqueSGMT * mntCommissionVirementGrandMnt);

				montant_tvaVirementMemeAgence = (nbreVirementMemeAgence * montant_tvaVirementPetitMnt);

				montant_tvaVirementAutreAgence_Banque = (nbreVirementAutreAgence_Banque * montant_tvaVirementPetitMnt);

				montant_tvaVirementAutreBanqueSGMT = (nbreVirementAutreBanqueSGMT * montant_tvaVirementGrandMnt);

				commissionTotal =
						mntCommissionVirementMemeAgence + mntCommissionVirementAutreAgence_banque
				...
				if (commissionTotal < Constants.MIN_COMMISSION) {
					...
					if (mntVirementMemeAgence > 0 && etatCommissionPercu == false) {
						...
						mntCommissionVirementAutreAgence_banque = 0;
						montant_tvaVirementAutreAgence_Banque = 0;
						mntCommissionVirementAutreBanqueSGMT = 0;
						montant_tvaVirementAutreBanqueSGMT = 0;
						etatCommissionPercu = true;

					} else if (mntVirementAutreAgence_Banque > 0 && etatCommissionPercu == false) {

						mntCommissionVirementMemeAgence = 0;
						montant_tvaVirementMemeAgence = 0;
						mntCommissionVirementAutreAgence_banque = Constants.MIN_COMMISSION.longValue();
						montant_tvaVirementAutreAgence_Banque =
								Long.valueOf((long) (Constants.MIN_COMMISSION.longValue() * 0.19));
						mntCommissionVirementAutreBanqueSGMT = 0;
						montant_tvaVirementAutreBanqueSGMT = 0;
						etatCommissionPercu = true;
				...
				if (nbreVirementAutreAgence_Banque > 0) {
					...
					fluxComptVirement.setMntVirStrc(mntVirementAutreAgence_Banque);
					fluxComptVirement.setNbreVirStrc(nbreVirementAutreAgence_Banque);
					...
					long nbrExisteFlux =
							virementGlobalDAO.getNbreFluxComptVirement(fluxComptVirement.getNumSeqGvir(),
									fluxComptVirement.getCodStrcStrc(), dateComptable,
									fluxComptVirement.getTypeFluxVir());
					if (nbrExisteFlux == 0) {
				...
				if (nbreVirementAutreBanqueSGMT > 0) {
					...
					fluxComptVirement.setNumSeqFlux(numSeqFlux);
					fluxComptVirement.setCodStrcStrc(new Long(codAgenceInerSgmt));
					fluxComptVirement.setMntVirStrc(mntVirementAutreBanqueSGMT);
					fluxComptVirement.setNbreVirStrc(nbreVirementAutreBanqueSGMT);
				...
				if (mntVirementAutreAgence_Banque > 0) {

					VirementVo virementVoCreationCroExeVir = new VirementVo();
					virementVoCreationCroExeVir.setEtatTaxableClient(etatClientTaxable);
					virementVoCreationCroExeVir.setGlobalVirement(globalVirement);
					virementVoCreationCroExeVir.setDateComptableAgence(dateComptable);
					virementVoCreationCroExeVir.setValueDateRecue(dateValeurAutreAgence_Banque);
					virementVoCreationCroExeVir.setMontant_commissionRecue(mntCommissionVirementAutreAgence_banque);
					virementVoCreationCroExeVir.setTvaRecue(montant_tvaVirementAutreAgence_Banque + "");
					virementVoCreationCroExeVir.setValueDateComRecue(dateValeurCommission);
					virementVoCreationCroExeVir.setMontantGlobalByCB(mntVirementAutreAgence_Banque);
					virementVoCreationCroExeVir.setNbreGlobalByCB(nbreVirementAutreAgence_Banque);
				...
				if (fluxComptVirements.size() > 0) {
					...
					for (FluxComptVirement fluxComptVirement : fluxComptVirements) {
						...
						if (fluxComptVirement.getCodStrcStrc().longValue() != Long.valueOf(900).longValue()) {
							...
							virementVo3.setNumRefCro(Long.valueOf(operationMoyPayCompteDOAutreAgence.getNumOperOmp()));
							virementVo3.setCodStrcStrc(operationMoyPayCompteDOAutreAgence.getStructureInitiatrice()
									.getCodStrcStrc());
							virementVo3.setDateOperation(operationMoyPayCompteDOAutreAgence.getDatOperOmp());
							virementVo3.setDateValeurCro(operationMoyPayCompteDOAutreAgence.getDatValOmp());
							virementVo3.setDateValeurCom(operationMoyPayCompteDOAutreAgence.getDateValeurCommission());
							virementVo3.setCodTachTach(operationMoyPayCompteDOAutreAgence.getTache().getTacheId()
									.getCodTachTach());
							virementVo3.setCodRefcOmp(operationMoyPayCompteDOAutreAgence.getCodRefcOmp());
							...
							virementVo4.setProduit(produitADT);
							...
							virementVo4.setNumRefCro(Long.valueOf(operationMoyPayCompteDOAutreAgence.getNumOperOmp()));
							virementVo4.setCodStrcStrc(operationMoyPayCompteDOAutreAgence.getStructureInitiatrice()
									.getCodStrcStrc());
							virementVo4.setDateOperation(operationMoyPayCompteDOAutreAgence.getDatOperOmp());
							virementVo4.setDateValeurCro(operationMoyPayCompteDOAutreAgence.getDatValOmp());
							virementVo4.setDateValeurCom(operationMoyPayCompteDOAutreAgence.getDateValeurCommission());
							virementVo4.setCodTachTach(operationMoyPayCompteDOAutreAgence.getTache().getTacheId()
									.getCodTachTach());
							virementVo4.setCodRefcOmp(operationMoyPayCompteDOAutreAgence.getCodRefcOmp());
							virementVo4 = (VirementVo) virementService.createCroReceptionVirementParAgence(virementVo4);

						} else if (fluxComptVirement.getCodStrcStrc().longValue() == Long.valueOf(900).longValue()) {
							if (fluxComptVirement.getTypeFluxVir().longValue() == 0) {
								...
								virementVo3.setNumRefCro(Long.valueOf(operationMoyPayCompteDOAutreBanque
										.getNumOperOmp()));
								virementVo3.setCodStrcStrc(operationMoyPayCompteDOAutreBanque.getStructureInitiatrice()
										.getCodStrcStrc());
								virementVo3.setDateOperation(operationMoyPayCompteDOAutreBanque.getDatOperOmp());
								virementVo3.setDateValeurCro(operationMoyPayCompteDOAutreBanque.getDatValOmp());
								virementVo3.setDateValeurCom(operationMoyPayCompteDOAutreBanque
										.getDateValeurCommission());
								virementVo3.setCodTachTach(operationMoyPayCompteDOAutreBanque.getTache().getTacheId()
										.getCodTachTach());
								virementVo3.setCodRefcOmp(operationMoyPayCompteDOAutreBanque.getCodRefcOmp());
								virementVo3 = (VirementVo) virementService.createCroEnvoiVirement(virementVo3);
								...
								virementVo3.setMontantGlobalByStructure(fluxComptVirement.getMntVirStrc().longValue());
								virementVo3.setCodRefInter(codRefInter);
								virementVo3.setNumRefCro(Long.valueOf(operationMoyPayCompteDOAutreBanqueSGMT
										.getNumOperOmp()));
								virementVo3.setDateOperation(operationMoyPayCompteDOAutreBanqueSGMT.getDatOperOmp());
								virementVo3.setCodStrcStrc(operationMoyPayCompteDOAutreBanqueSGMT
										.getStructureInitiatrice().getCodStrcStrc());
								virementVo3.setDateValeurCro(operationMoyPayCompteDOAutreBanqueSGMT.getDatValOmp());
								virementVo3.setDateValeurCom(operationMoyPayCompteDOAutreBanqueSGMT
										.getDateValeurCommission());
								virementVo3.setCodTachTach(operationMoyPayCompteDOAutreBanqueSGMT.getTache()
										.getTacheId().getCodTachTach());
								virementVo3.setCodRefcOmp(operationMoyPayCompteDOAutreBanqueSGMT.getCodRefcOmp());
				...
				for (DetailVirement detailVirementObj : list_Details) {
					...
					// ***** Bénéficiaire BNA *****//
					if (codeBanque.equals("03")
							&& new Long(codeAgenceCompteBenif).longValue() == globalVirement.getStructure()
						...
						if (etatValiditeBenif == false) {
							...
							virementVoReaf =
									(VirementVo) virementService.createCroReaffectationRejetsVirement(virementVoReaf);
					...
					// ***** Bénéficiaire autre agence ou non BNA *****//
						...
						if (detailVirementObj.getMntDetvDetv().longValue() < Constants.MONTANT_VIREMENT_SGMT
							...
							virementVoCreationFile.setMntVirementADT(mntVirementAutreAgence_Banque);
							virementVoCreationFile.setNbrVirementADT(nbreVirementAutreAgence_Banque);
						...
						// **** Virement autre Banque SGMT****//
							...
							virSgmt.setCodBdBank(new Short("3"));
							if (globalVirement.getContratCpt().getNomIntiCcpt().length() > 30) {
								virSgmt.setNomNomdVir(globalVirement.getContratCpt().getNomIntiCcpt().substring(0, 30));
							} else {
								virSgmt.setNomNomdVir(globalVirement.getContratCpt().getNomIntiCcpt());
							}
							...
							if (detailVirementObj.getNomBenDetv().length() > 30) {
								virSgmt.setNomNombVir(detailVirementObj.getNomBenDetv().substring(0, 30));
							} else {
								virSgmt.setNomNombVir(detailVirementObj.getNomBenDetv());
							}

							virSgmt.setMntVirVir(detailVirementObj.getMntDetvDetv());
							if (detailVirementObj.getMotiDetvDetv().length() > 45) {
								virSgmt.setLibMotifVir(detailVirementObj.getMotiDetvDetv().substring(0, 45));
							} else {
								virSgmt.setLibMotifVir(detailVirementObj.getMotiDetvDetv());
							}
			...
			StringBuffer text = new StringBuffer("Erreur dans ExecutionDoVirMasseSalaireTrt : ");
			text.append(e.toString());
			erreur.setCode("100");
			erreur.setDescription(text.toString());
			erreur.setKey("ExecutionDoVirMasseSalaireTrt");
			virementVo.addError(erreur);
			logger.error("Erreur au niveau ExecutionDoVirMasseSalaireTrt : ", e);
			virementVo.setMessageValidation("Probléme dans ExecutionDoVirMasseSalaireTrt");
