...
public class Cro2252Trt extends Traitement {
...
//		numRefCro = formatDateFile.format(date.getTime());
		...
		this.setLibRefCro("Souscription Assurance FAIEZ");
		String sysdate = null;
		...
		int compteur = 1;
		Long nextRef = 0L;
		if(operationMoyPay.getStructureInitiatrice().getCodStrcStrc() == 120L) {
			refIntSg1 = null;
		}else {
			String res = assVieEpargneDAO.getMaxRefInter(operationMoyPay.getStructureInitiatrice().getCodStrcStrc(), operationMoyPay.getTache().getTacheId().getCodOperOper(), sysdate);
			if (res == null) {
				if(operationMoyPay.getStructureInitiatrice().getCodStrcStrc() == 1L) {
					compteur = 3;
					String ag = StrHandler.lpad(operationMoyPay.getStructureInitiatrice().getCodStrcStrc().toString(), '0', 3);
