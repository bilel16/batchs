...
public class Cro962EncaissementLcnPayeeTrt extends Traitement {
	...
	public Cro962EncaissementLcnPayeeTrt() {
	}
	...
	public IValueObject perform(IValueObject vo) {
		...
		try {
			...
			traitementConditionBanque.setCodOperOper("" + Constants.COD_OPERATION_ENCAISSEMENT_EFFET_PAYEE);
			...
			operationMoyPay.setLibObjOpOmp(" ENCAISSEMENT LCN PAYEE");
			...
			oper.setCodOperOper(Constants.COD_OPERATION_ENCAISSEMENT_EFFET_PAYEE);
			Tache tache = new Tache();
			tache.setOperation(oper);
			TacheId tacheId = new TacheId();
			tacheId.setCodOperOper(Constants.COD_OPERATION_ENCAISSEMENT_EFFET_PAYEE);
			...
			if (valueDateRecue != null && valueDateRecue.length() > 0) {
				dateValOmp = DateHandler.strToDate(valueDateRecue);
			} else {
			...
			operationMoyPay.setLibMotfOmp("ENCAISSEMENT EFFET ");

			// 02. insertion operation moyen paiement
			operationMoyPay.setNumRibOmp(reglementEffetVo.getEffetRecu().getRibTir());
			operationMoyPay.setNomNomdOmp(reglementEffetVo.getEffetRecu().getNomTir());
		...
		/*
		 * } catch (Exception e) { logger.info(e.getMessage()); com.oxia.fwk.core.Error erreur = new
		 * com.oxia.fwk.core.Error(); StringBuffer text = new
		 * StringBuffer("Erreur dans Cro962EncaissementLcnPayeeTrt : "); text.append(e.toString());
		 * erreur.setCode("100"); erreur.setDescription(text.toString());
		 * erreur.setKey("Cro962EncaissementLcnPayeeTrt"); reglementEffetVo.addError(erreur);
		 * logger.error("Erreur au niveau Cro962EncaissementLcnPayeeTrt : ", e); return (reglementEffetVo);
		 * 
		 * }
		 */
	...
	public void genCroText(ValueObject vo) {
		...
		ContratCpt contratCptBen = reglementEffetVo.getContratCpt();
		operation.setCodOperOper(Constants.COD_OPERATION_ENCAISSEMENT_EFFET_PAYEE);
		...
		this.setLibRefCro("ENCAISSEMENT EFFET");
