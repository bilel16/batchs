...
public class BatchReaffRejetsNSIFrame extends JFrame implements Serializable {
	...
	public BatchReaffRejetsNSIFrame frame_Instance = null;
		...
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					...
					BatchReaffRejetsNSIFrame frame = new BatchReaffRejetsNSIFrame();
	 ...
	 * Create the frame.
	 */
	public BatchReaffRejetsNSIFrame() {

		setTitle("Batch Réaffectation Virements Sièges ");
		...
		btnExcuter.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (radio_Virement.isSelected()) {
					...
					new Thread(new MoulinetteReaffRejetsNSITest(frame_Instance)).start();
		...
		radio_Virement = new JRadioButton("Réaffectation Rejets Virements");
		radio_Virement.setSelected(true);
		buttonGroup.add(radio_Virement);
		radio_Virement.setBounds(35, 94, 209, 23);
		...
		textFieldAgence.setBounds(123, 138, 86, 20);
		panel.add(textFieldAgence);

		JLabel lblAgence = new JLabel("AGENCE :");
		lblAgence.setBounds(40, 137, 73, 23);
		panel.add(lblAgence);
