...
import com.bna.smile.batch.moulinette.MoulinetteCommissionFraisTenueCompteControlM;
...
public class MoulinetteCommissionTenueComptePackControlMTest implements Runnable {

	private static final Logger logger = Logger.getLogger(MoulinetteCommissionTenueComptePackControlMTest.class);

	private static final SimpleDateFormat formaterDate = new SimpleDateFormat("dd/MM/yyyy");

	// *********************************//

	public MoulinetteCommissionTenueComptePackControlMTest() {
		super();
	}
	...
	public void run() {
		...
		if (batchVo.getListeTraceBatch() != null && batchVo.getListeTraceBatch().size() > 0) {
			System.out.println("Une autre moulinette perception frais tenue compte pack est en cours d'exécution");
			...
			if (traceBatch.getNumSeqTrc() != null) {
				...
				if (listAgences != null && listAgences.size() > 0) {
					...
					for (Object result : listAgences) {
						...
						// ********Batch *************//
						MoulinetteCommissionFraisTenueCompteControlM moulinette =
								new MoulinetteCommissionFraisTenueCompteControlM(virementVo);
				...
				System.out.println("Moulinette Commission Frais Pack terminée avec succès");
