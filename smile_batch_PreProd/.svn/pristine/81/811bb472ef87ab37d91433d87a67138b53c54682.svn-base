...
public class PrelevementTroisiemeTrancheEpargneEtudeResiduTrt extends Traitement {

	Log logger = new Log(PrelevementTroisiemeTrancheEpargneEtudeResiduTrt.class);
	...
	public IValueObject perform(IValueObject vo) {
		...
		ListFaiezResiduVo listContratFaiezVo = (ListFaiezResiduVo) vo;
		List<TraceAssuranceFaiezVo> list = listContratFaiezVo.getList();
		Calendar date = Calendar.getInstance();
		String sysdate = null;
		SimpleDateFormat formatDateFile = new SimpleDateFormat("dd/MM/yyyy");
		sysdate = formatDateFile.format(date.getTime());
		try {
			for (TraceAssuranceFaiezVo contratFaiezVo : list) {
...
//				// 03. setting structure receptrice
				...
				operationMoyPayCredit.setMontDinOmp(contratFaiezVo.getMont_trch3_cat());

				operationMoyPayCredit.setMontApreOmp(contratCptBen.getMontSoldCcpt() + contratFaiezVo.getMont_trch3_cat());
				...
				// Checking Devise
				contratCptSold.setSolde(contratFaiezVo.getMont_trch3_cat());
...
//				operationMoyPayDebit.setStructureReceptrice(structureRecepD);
				...
				// 05. setting devise et montant
				operationMoyPayDebit.setMontDinOmp(contratFaiezVo.getMont_trch3_cat());

				operationMoyPayDebit.setMontApreOmp(contratCptDeb.getMontSoldCcpt() - contratFaiezVo.getMont_trch3_cat());
				...
				contratCptSoldDebit.setSolde(contratFaiezVo.getMont_trch3_cat());
				...
				assVieEpargneDAO.UpdateTraceTrancheFaiezRésiduePayée(contratFaiezVo, sysdate, 3);
			}
