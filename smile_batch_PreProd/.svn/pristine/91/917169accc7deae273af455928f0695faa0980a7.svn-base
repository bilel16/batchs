...
public class MAJNSITrt extends Traitement {
	...
	public MAJNSITrt(BatchMAJNSIFrame mainFrame) {
		super();
	...
	public IValueObject perform(IValueObject vo) {
		...
		try {
			...
			try {
				...
				mAJNSIVo.setMainFrame(mainFrame);
				mAJNSIVo = (MAJNSIVo) guichetService.MAJNSIFileCopy(mAJNSIVo);
			} catch (Exception e) {
			...
			//135 299999
			SwingInfoVo infoVo1 = new SwingInfoVo();
			infoVo1.setStructure("Agence Non NSI");
			infoVo1.setEtat(Constants.STATUT_EN_COURS_INSERT);
			infoVo1.setDateComptable(formatDate.format(new Date()));
			mainFrame.addOrUpdateEtat(infoVo1);
			// fin 135 299999
		...
		//	criteriaAgVirNsi.add(expressionVirNsi.eq("boolIns", 9L)); for new batch 135
			...
			gestionStatistique(formatDate.parse(formatDate.format(new Date())), strc,
					agNonNsi.getNbTotalOperations(), agNonNsi
							.getNbOperationsPosi(), agNonNsi.getNbOperationsInter(),
							agNonNsi.getNbOperationsRej(), agNonNsi
							.getSommeOp(),
							agNonNsi
							.getSommeOpDebit());
			
			SwingInfoVo infoVo = new SwingInfoVo();
			infoVo.setStructure("Agence Non NSI");
			infoVo.setEtat(Constants.STATUT_EN_TERMINE);
			infoVo.setDateComptable(formatDate.format(new Date()));
			String info=" Nombre total des opérations = " + agNonNsi.getNbTotalOperations()
					+ "  ; Nombre des opérations comptes internes = " +  agNonNsi.getNbOperationsInter()
					+ " ; Nombre des opérations positionnées  = " + agNonNsi
					.getNbOperationsPosi() + " ; Nombre des rejets = " + agNonNsi.getNbOperationsRej()
			+ "  ;  pour la somme Credit de : " + (agNonNsi
							.getSommeOp()) + " Dinars"
			     + "  ;  pour la somme Debit de : " + (agNonNsi
					.getSommeOpDebit()) + " Dinars";
			infoVo.setInfo(info);
			mainFrame.getBtnExcuter().setEnabled(false);
			mainFrame.addOrUpdateEtat(infoVo);
