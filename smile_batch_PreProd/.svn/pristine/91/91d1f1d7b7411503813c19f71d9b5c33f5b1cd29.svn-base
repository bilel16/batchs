 ...
 * Cont, les comptes des personnes decédés et las personnes ayant>70 ans
 * 
 * @author Y.BOUSSEN
 * @since 30/12/2010
 */

public class ResiliationAutoRenouvellementAutoTrt extends Traitement {

	Logger logger = Logger.getLogger(ResiliationAutoRenouvellementAutoTrt.class);

	public ResiliationAutoRenouvellementAutoTrt() {
	}
	...
	public IValueObject perform(IValueObject vo) {
		...
		try {
			...
			e.printStackTrace();
			StringBuffer text = new StringBuffer("Erreur dans ResiliationAutoRenouvellementAutoTrt : ");
			...
			System.out.println(
					"  /*************** Fin du Batch : ResiliationAutoRenouvellementAutoTrt (Assurance vie)  *****************/");
			System.out.println("  ");
		}
	...
	private void genererFichier(List lren, List lres) throws IOException {
		...
		try {
			Date toDay = new Date();
			String f = "C:\\" + "MAGHREBIA_RENOUV_RESIL" + (toDay.getYear() + 1900) + "_"
			...
			lres = assuranceVoyageDAO.getlisteDetailAdhesionVie(DateHandler.addMonth(toDay, -6), new Date(),
					Constants.COD_ASS_MAGHREBIA, Constants.COD_ETA_RESIL_ASSUR_VIE);
			...
			if (lren != null && lren.size() > 0) { // /*** les renouvellements
				for (Iterator it1 = lren.iterator(); it1.hasNext();) {
					...
					ligne = "11" + StrHandler.rpad(adhesionAssVie.getClient().getPersonne().getNomNomPers(), ' ', 30);
					if (adhesionAssVie.getClient().getPersonne().getNomPrnPers() != null) {
						ligne = ligne
								+ StrHandler.rpad(adhesionAssVie.getClient().getPersonne().getNomPrnPers(), ' ', 30);
					} else if (adhesionAssVie.getClient().getPersonne().getNomRsPers() != null) {

						String NomRs = adhesionAssVie.getClient().getPersonne().getNomRsPers();
						if (NomRs.length() <= 30) {
							ligne = ligne + StrHandler.rpad(NomRs, ' ', 30);

						} else {
							ligne = ligne + StrHandler.rpad(NomRs.substring(0, 30), ' ', 30);
						}

					}
					ligne = ligne + datNaisPers
							+ StrHandler.lpad(adhesionAssVie.getClient().getPersonne().getNumPcePers(), '0', 8)
							+ primitiveVO.getVString() + datEffetAdh
							+ StrHandler.lpad(adhesionAssVie.getTarifAssVie().getMontPrmaTass().toString(), '0', 7)
							+ StrHandler.lpad(adhesionAssVie.getTarifAssVie().getMontComTass().toString(), '0', 7)
							+ StrHandler.lpad(adhesionAssVie.getTarifAssVie().getMontPrmgTass().toString(), '0', 7);
					Long ll = Long
							.valueOf(Math.round(new Double(adhesionAssVie.getTarifAssVie().getMontComTass()).intValue()
									* Constants.TAUX_RETENU_A_LA_SOURCE_ASSUR_VIE / 100));
					ligne = ligne + StrHandler.lpad(ll.toString(), '0', 7) + StrHandler
							.lpad(adhesionAssVie.getContratCpt().getContratCptId().getCodPrdPrd().toString(), '0', 4);
					out.write(ligne);
			...
			if (lres != null && lres.size() > 0) { // /*** les résiliations
				for (Iterator it1 = lres.iterator(); it1.hasNext();) {
					...
					AdhesionAssVie adhesionAssVie = (AdhesionAssVie) searchEngine.get(AdhesionAssVie.class,
							detailAdhesion.getAdhesionAssVie().getNumSeqAdh());
			...
			String ligneFinale = "99" + StrHandler.lpad(String.valueOf(totalNbrLignes), '0', 7)
	...
	private void gestionStatistiqueResilRenouv(int nbrResiliation, int nbrRenouvellement) {
		...
		try {
			...
			batchStatPlacement.setLibExtrBats(
					nbrResiliation + " Contrat(s) Résilié(s) et   " + nbrRenouvellement + " Contrat(s) Renouvelé(s)");
