...
public class DebloquerMntTrt extends Traitement{
    public DebloquerMntTrt() {
    }
    ...
    public IValueObject perform(IValueObject vo) {
        ...
        try {
            ...
            contratCpt=montantBlocage.getContratCpt();
           
            /*MAJ des blocage*/
            for (Iterator it = montantBlocage.getListBlocageChoisi().iterator(); it.hasNext(); ) {
            Blocage blocage=(Blocage)it.next();
            blocage.setDatFinBloc(new Date());
            crudService.update(blocage);
            montantBlocage.setBlocage(blocage);
            this.sychronisationPascal(montantBlocage);  
            
            }
            
            /*MAj du solde contrat Compte*/
            ...
            StringBuffer text = 
                new StringBuffer("Erreurdéblocage montant ");
            text.append(e.toString());
            erreur.setCode("200");
            erreur.setDescription(text.toString());
            erreur.setKey("Débloquermontant");
    ...
    public void genererSynchronisationPascal(ValueObject vo) { 
        ...
        String reference="CA                  ";                    
        String montantBloc=StrHandler.lpad(montantBlocage.getBlocage().getMntBlocBloc().toString(),'0',15); 
