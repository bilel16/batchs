...
public class CloturerDomMoyPaiTrt extends Traitement{
    public CloturerDomMoyPaiTrt() {
    }
        ...
        try {
            if (this.checkClotureJournee()) {
                if ((journeeStructureDomaine.getCodStatJsd().equals(Constants.ETAT_JSDOM_COURCLO)) || 
                    ...
                    erreur.setKey("CloturerDomMoyPaiTrt");
            ...
            journStrucDomVo.addError(erreur);
            logger.error(" *** Erreur lors de la CloturerDomMoyPaiTrt concernant l'agence " + 
    ...
    public void genCroText(ValueObject vo) {
        ...
        try {
            ...
            if (journStrucDomVo.getListeOperVirement() != null && 
                journStrucDomVo.getListeOperVirement().size() > 0) {
                for (Iterator it = 
                     journStrucDomVo.getListeOperVirement().iterator(); 
                     it.hasNext(); ) {
            ...
            com.oxia.fwk.core.Error erreur = new com.oxia.fwk.core.Error();
            StringBuffer text = 
                new StringBuffer("Erreur dans CloturerDomMoyPaiTrt : ");
            text.append(e.toString());
            erreur.setCode("200");
            erreur.setDescription(text.toString());
            erreur.setKey("ClotureDomaineTrt");
