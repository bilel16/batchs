...
import com.bna.smile.batch.moulinette.MoulinetteRenouvellementAssuranceVieDecouvertAMI;
...
public class MoulinetteRenouvellementAdhesionVieDecouvertAMITest implements Runnable {

	private static final Logger logger = Logger.getLogger(MoulinetteRenouvellementAdhesionVieDecouvertAMITest.class);
	private BatchRenouvellementAssuranceVieDecouvertAMIFrame frame = null;
	private static final SimpleDateFormat formaterDate = new SimpleDateFormat("dd/MM/yyyy");

	// *********************************//

	public BatchRenouvellementAssuranceVieDecouvertAMIFrame getFrame() {
		return frame;
	}

	public void setFrame(BatchRenouvellementAssuranceVieDecouvertAMIFrame frame) {
		this.frame = frame;
	}

	public MoulinetteRenouvellementAdhesionVieDecouvertAMITest() {
		super();
	}

	public MoulinetteRenouvellementAdhesionVieDecouvertAMITest(BatchRenouvellementAssuranceVieDecouvertAMIFrame frame) {
		super();
		this.frame = frame;
	}
	...
	public void run() {
		...
		if (batchVo.getListeTraceBatch() != null && batchVo.getListeTraceBatch().size() > 0) {
			frame.getEnd_exec_label().setText("Une autre moulinette renouvellement assurance vie découvert est en cours d'exécution");
			...
			if (traceBatch.getNumSeqTrc() != null) {
				...
				List listAgences = virementGlobalDAO.getListAgencesBNA();
				Structure agence = new Structure();
				Date dateComptableAgence = null;
				ParamAdhesion paramAdhesionVo = new ParamAdhesion();
				if (listAgences != null && listAgences.size() > 0) {
					logger.info("*** Debut MoulinetteRenouvellementResiliation ***");
					...
					for (Object result : listAgences) {
						...
						paramAdhesionVo.setStructure(agence);
						paramAdhesionVo.setDateComptable(dateComptableAgence);
						...
						// ********Batch *************//
						MoulinetteRenouvellementAssuranceVieDecouvertAMI moulinette = new MoulinetteRenouvellementAssuranceVieDecouvertAMI(paramAdhesionVo);
				...
				frame.getEnd_exec_label().setText("Moulinette renouvellement assurance vie découvert terminée avec succès");
				frame.getBtnExcuter().setEnabled(true);
				logger.info("*** FIN MoulinetteRenouvellementAdhesionVieDecouvertAMITest ***");
			}
		}
	}
}
