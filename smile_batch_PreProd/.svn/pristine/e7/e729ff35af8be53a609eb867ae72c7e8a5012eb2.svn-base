...
public class CreationCROExecutionVirementSuccessionTrt extends Traitement {
	...
	public CreationCROExecutionVirementSuccessionTrt() {
	}
	...
	public IValueObject perform(IValueObject vo) {
		...
		try {
			...
			operation.setCodOperOper(Constants.COD_OPER_EXECUTION_VIREMENT_SUCCESSION);
			...
			operationMoyPayCompteDO.setLibObjOpOmp("EXECUTION VIREMENT D'UNE SUCCESSION " + motifEnvoi);
			...
			tacheId.setCodOperOper(Constants.COD_OPER_EXECUTION_VIREMENT_SUCCESSION);
			tache.setOperation(operation);

			// 11. 1 Cas de Virement Emis
			if (Constants.COD_OPER_EXECUTION_VIREMENT_SUCCESSION.equals(operation.getCodOperOper())) {
			...
			if (montant_commision != 0) {
				...
				// **** A verifier **** //
				nomencElemtCondition.setCodNecdNecd(Constants.COD_NECD_NECD_1228 + "");
			...
			operationMoyPayCompteDO.setLibMotfOmp("EXECUTION VIREMENT D'UNE SUCCESSION");
			...
			StringBuffer text = new StringBuffer("Erreur dans CreationCROExecutionVirementSuccessionTrt : ");
			text.append(e.toString());
			erreur.setCode("100");
			erreur.setDescription(text.toString());
			erreur.setKey("CreationCROExecutionVirementSuccessionTrt");
			virementVo.addError(erreur);
			logger.error("Erreur au niveau CreationCROExecutionVirementSuccessionTrt : ", e);
			virementVo.setMessageValidation("Probléme dans CreationCROExecutionVirementSuccessionTrt");
	...
	public void genCroText(ValueObject vo) {
		logger.info("starting Creation CRO DO 1228");
		...
		this.setLibRefCro("EXECUTION VIREMENT D'UNE SUCCESSION " + motifRefCro);
		...
		montantGlobal = montantGlobal + montant_commision + montant_tva;
		...
		// Montant NET
		long mntNet = 0;
		// mntNet = montantGlobal - montant_commision - montant_tva;
		mntNet = virementVo.getMontantGlobalByCB();
		cro.append("MNT_NET_VEM=");
		cro.append(mntNet + ";");
