...
public class MAJAutLigneTrt extends Traitement {

	public MAJAutLigneTrt() {
	}
...
String dd=ligne.trim().substring(28, 34);
			...
			strc.setCodStrcStrc(contratCptId.getCodStrcStrc());
				...
				cptPret = (ContratCpt) getContratCptByIdCmd.execute(cptPret);
					...
					if (cptPret == null) {
						bufWriter.write(" Compte inexistant "+ligne);
						bufWriter.newLine();

					} else {
						
						
								mnt = Long.valueOf(ligne.trim().substring(13, 28));
								mnt=new Double(mnt).longValue();
								depPerso(cptPret, formatDate.parse(dd), mnt);
			...
			StringBuffer text = new StringBuffer("Erreur dans MAJNSIAut-perf : ");
	...
	public void depPerso(ContratCpt contratCpt, Date dateEch,Long mnt) throws Exception {
		DepPersonnel depPersonnel = new DepPersonnel();
		String codeAgence = contratCpt.getContratCptId().getCodStrcStrc().toString();
		...
		depPersonnel.setMontCredCredeps(mnt);
		depPersonnel.setMontAutCredeps(mnt);
		depPersonnel.setDatEchCredeps(dateEch);
		...
		String cpt =
				StrHandler.lpad("" + contratCpt.getContratCptId().getCodStrcStrc(), '0', 3)
						+ StrHandler
								.lpad("" + contratCpt.getContratCptId().getCodPrdPrd(), '0', 4)
						+ StrHandler.lpad("" + contratCpt.getContratCptId().getNumCcptCcpt(), '0',
								6);
		depPersonnel.setNumCptCredeps(cpt);
		depPersonnel.setCodStrcStrc(contratCpt.getContratCptId().getCodStrcStrc());
		depPersonnel.setNumSeqPers(contratCpt.getClient().getPersonne().getNumSeqPers());
		...
		ContratCpt contratCptU =
				(ContratCpt) searchEngine.loadForUpdate(ContratCpt.class,contratCpt
						.getContratCptId());
		contratCptU.setDatEautCcpt(depPersonnel.getDatEchCredeps());
