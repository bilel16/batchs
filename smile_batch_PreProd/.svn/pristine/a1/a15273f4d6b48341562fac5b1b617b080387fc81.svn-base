...
public class BatchMAJNSIFrame extends JFrame implements Serializable {
	...
	public BatchMAJNSIFrame frame_Instance = null;
		...
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					...
					BatchMAJNSIFrame frame = new BatchMAJNSIFrame();
					frame.setVisible(true);
					frame.btnExcuter.setEnabled(false);
					new Thread(new MoulinetteMAJNSITest(frame)).start();
	 ...
	 * Create the frame.
	 */
	public BatchMAJNSIFrame() {

		setTitle("Batch Virements Sièges : MAJ NSI ");
		...
		btnExcuter.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (radio_Virement.isSelected()) {
					...
					new Thread(new MoulinetteMAJNSITest(frame_Instance)).start();

				}

			}
		});
		btnExcuter.setBounds(90, 189, 90, 23);
		panel.add(btnExcuter);

		radio_Virement = new JRadioButton("MAJ NSI");
		radio_Virement.setSelected(true);
		buttonGroup.add(radio_Virement);
		radio_Virement.setBounds(35, 94, 109, 23);
		panel.add(radio_Virement);

		end_exec_label = new JLabel("Lacement du Batch");
