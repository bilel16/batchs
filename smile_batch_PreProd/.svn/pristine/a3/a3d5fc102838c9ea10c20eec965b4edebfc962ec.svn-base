...
import com.bna.smile.model.banqueAssurance.traitement.EnvoiFichierCapiTrt;
...
public class MoulinetteEnvoiFichierCapi implements Runnable {

	String[] args;

	private static final Log LOGGER = LogFactory.getLog(MoulinetteEnvoiFichierCapi.class.getSimpleName());
	...
	public void run() {
		...
		if (batchVo.getListeTraceBatch() != null && batchVo.getListeTraceBatch().size() > 0) {
			LOGGER.error("Une autre moulinette envoi fichier CAPI est en cours d'exécution");
			return;
		} else {
			/* Insertion trace **** */
			...
			if (traceBatch.getNumSeqTrc() != null) {
				...
				EnvoiFichierCapiTrt evoisFichierTrt = new EnvoiFichierCapiTrt();
				try {
					...
					if (args.length == 0) {
						...
						if (dateExtract == null) {
							LOGGER.error(
									"MoulinetteEnvoiFichierCapi : mauvais format de la date passé en parametre");
						...
						LOGGER.error("MoulinetteEnvoiFichierCapi : probleme des parametres du batch ");
						throw new InterruptedException();
					}
				} catch (Exception e) {
					LOGGER.error("MoulinetteEnvoiFichierCapi", e);
					return;
				}
				...
				LOGGER.info("Moulinette envoi fichier Capi terminée avec succès");
